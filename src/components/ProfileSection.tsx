...
export default function ProfileSection({ state, updateState, onOpenSettings, onLogout, onClose }: ProfileSectionProps) {
  ...
  const handleSave = async () => {
    ...
    updateState((prev) => ({
      ...prev,
      userProfile: { ...prev.userProfile, name: name.trim(), avatarUrl: tempAvatar },
    }));
    try {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1 * 1024 * 1024) {
      showToast('Image too large (max 1 MB).', 'error');
      return;
    }
    ...
    reader.onload = (ev) => {
      ...
      if (b64) {
        setTempAvatar(b64);
        showToast('Image loaded — Save to keep it.', 'success');
      }
    };
  ...
  const handleRemovePhoto = () => {
    ...
    showToast('Photo cleared — Save to confirm.', 'info');
  };
    ...
    <div className="card p-6 md:p-7 space-y-5" id="secure-profile-card">
      ...
      <div className="flex flex-col items-center text-center">
        <div className="relative">
          <div className="w-[76px] h-[76px] rounded-full bg-[var(--surface-2)] border border-[var(--line)] overflow-hidden flex items-center justify-center text-[var(--ink)] text-[22px] font-bold">
            {tempAvatar ? (
              <img src={tempAvatar} alt={name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            ) : (
              firstLetter
            )}
            {isEditing && (
          ...
          {!isEditing && (
            <span
              className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-[var(--success)] border-2 border-[var(--surface)]"
              aria-hidden
            />
          )}
        </div>
        ...
        {isEditing ? (
          <div className="w-full max-w-[260px] mt-4 space-y-2.5">
            {tempAvatar && (
              <button onClick={handleRemovePhoto} className="btn-ghost w-full justify-center text-[12px]">
            ...
            />
            <button onClick={handleSave} className="btn-primary w-full justify-center inline-flex items-center gap-1.5">
              <Save size={12} /> Save
            </button>
          </div>
        ) : (
          <div className="mt-3">
            <h3 className="text-[15px] font-bold text-[var(--ink)] tracking-tight">
              {state.userProfile?.name || profileFallback}
            </h3>
            <p className="mono text-[11px] text-[var(--ink-2)] mt-0.5">{state.userProfile?.email || 'Local vault'}</p>
            <span className="mt-2 inline-flex mono text-[10px] px-2 py-0.5 rounded-full border border-[var(--line)] bg-[var(--surface-2)] text-[var(--ink-2)]">
              Premier member
            </span>
          </div>
        )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3 py-1">
          <span className="w-8 h-8 rounded-full bg-[var(--surface-2)] border border-[var(--line)] flex items-center justify-center text-[var(--ink-2)] shrink-0">
            <Mail size={13} />
          </span>
        ...
        <button onClick={onOpenSettings} className="w-full flex items-center justify-between py-2 text-left group">
          <span className="flex items-center gap-3">
            <span className="w-8 h-8 rounded-full bg-[var(--surface-2)] border border-[var(--line)] flex items-center justify-center text-[var(--ink-2)] group-hover:text-[var(--ink)]">
              <KeyRound size={13} />
            </span>
            <span>
              <span className="eyebrow block">Cloud vault</span>
              <span className="text-[12px] font-semibold text-[var(--ink)]">Encryption &amp; sync settings</span>
            </span>
          </span>
      ...
      <button
        ...
        className="btn-ghost w-full justify-center inline-flex items-center gap-2 text-[var(--danger)] hover:bg-[var(--danger-bg)] border-[var(--line)]"
      >
