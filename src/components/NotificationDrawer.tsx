        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5">
          {notifications.length === 0 ? (
            <div className="py-14 text-center">
              <div className="w-10 h-10 rounded-full bg-[var(--surface-2)] border border-[var(--line)] flex items-center justify-center mx-auto text-[var(--ink-3)]">
                <Bell size={16} />
              </div>
              <p className="eyebrow mt-3">All clear</p>
              <p className="text-[12px] leading-5 text-[var(--ink-2)] mt-1 max-w-[28ch] mx-auto">
                No alerts — your ledger is quiet.
              </p>
            </div>
          ) : (
            <>
              {notifications.map((notif) => {
                const tone =
                  notif.type === 'alert'
                    ? 'text-[var(--danger)] bg-[var(--danger-bg)] border-[var(--line)]'
                    : notif.type === 'reminder'
                      ? 'text-[var(--warning)] bg-[var(--warning-bg)] border-[var(--line)]'
                      : 'text-[var(--ink-2)] bg-[var(--surface-2)] border-[var(--line)]';
                return (
                  <div
                    key={notif.id}
                    className={`rounded-xl border p-3.5 ${notif.read ? 'bg-[var(--surface-2)] border-[var(--line)] opacity-70' : 'bg-[var(--surface)] border-[var(--line)]'}`}
                  >
                    ...
                    <div className="flex justify-end gap-2 mt-2.5 pt-2.5 border-t border-[var(--line)]">
                      {!notif.read && (
                        <button
                          ...
                          className="mono text-[11px] px-2.5 py-1 rounded-full border border-[var(--line)] bg-[var(--surface-2)] text-[var(--ink-2)] hover:text-[var(--ink)] inline-flex items-center gap-1"
                        >
                          <Check size={11} /> Read
                        </button>
                      )}
                      <button
                        ...
                        className="mono text-[11px] px-2.5 py-1 rounded-full border border-[var(--line)] bg-[var(--surface)] text-[var(--ink-3)] hover:text-[var(--danger)] inline-flex items-center gap-1"
                      >
