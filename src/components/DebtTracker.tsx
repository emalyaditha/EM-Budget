...
export default function DebtTracker({
  ...
  const validateDebtForm = (src: string, amtStr: string, date: string, sub: boolean) => {
    ...
    if (sub || amtStr) {
      if (!amtStr) errs.amount = 'Amount is required';
        ...
        if (isNaN(num)) errs.amount = 'Must be a number';
        else if (num <= 0) errs.amount = 'Must be positive';
      }
    }
    if (sub || date) {
      if (!date) errs.dueDate = 'Due date is required';
    }
    setErrors(errs);
  ...
  const handleCreateDebt = (e: React.FormEvent) => {
    ...
    if (!isValid) {
      ...
      showToast('error', 'Please resolve highlighted errors.');
      return;
    }
    ...
    setSubmitted(false);
    setErrors({});
    showToast('success', 'Debt registered.');
  };
  ...
  const handlePayDebtSubmit = (e: React.FormEvent) => {
    ...
    if (!debtItem) return;
    if (amountNum <= 0) {
      setPaymentError('Amount must be > 0.');
      return;
    }
    if (amountNum > debtItem.remainingAmount) {
      setPaymentError(`Exceeds remaining ${currency} ${debtItem.remainingAmount.toLocaleString()}`);
      return;
    }
    ...
    if (availableBalance < amountNum + chargeNum) {
      setPaymentError(
        `Insufficient balance. Required ${currency} ${(amountNum + chargeNum).toLocaleString()}, available ${currency} ${availableBalance.toLocaleString()}`,
      );
      return;
    }
  ...
  const handleIncreaseDebtSubmit = (e: React.FormEvent) => {
    ...
    onIncreaseDebt(
      increasingDebtId,
      amountNum,
      incTargetAccountId || undefined,
      (incTargetAccountType as 'cash' | 'card') || undefined,
    );
    setIncreaseAmount('');
    setIncTargetAccountId('');
    setIncTargetAccountType('');
    setIncreasingDebtId(null);
    showToast('success', 'Additional debt added.');
  };
  ...
  const overallClearedPercent =
    totalOriginalDebt > 0 ? Math.round(((totalOriginalDebt - totalRemainingDebt) / totalOriginalDebt) * 100) : 100;
  ...
  return (
    <div id="debt-tracker-vault-view" className="space-y-6">
      <div className="gradient-card p-6 overflow-hidden" style={{ background: 'var(--gradient-card-dark)' }}>
        <div className="flex flex-col sm:flex-row justify-between gap-4 relative z-10">
          ...
          <div className="flex flex-col items-start sm:items-end gap-1 relative z-10">
            <span className="pill !bg-white/10 !border-white/15 !text-white mono !text-[11px]">
              {overallClearedPercent}% repaid
            </span>
            <span className="mono text-[26px] font-bold tracking-tight text-white">
              {currency} {totalRemainingDebt.toLocaleString()}
            </span>
            <span className="mono text-[11px] text-white/60 block truncate">
              remaining · {currency}
              {totalOriginalDebt.toLocaleString()} original
            </span>
            <div className="h-2 w-full sm:w-48 mt-2 rounded-full bg-white/10 overflow-hidden">
              <div className="h-full mw-progress" style={{ width: `${overallClearedPercent}%` }} />
            </div>
          </div>
        </div>
      ...
      <div className="flex justify-between items-center">
        <div>
          <p className="eyebrow">Creditor registry</p>
          <p className="mono text-[11px] mt-1" style={{ color: 'var(--ink-3)' }}>
            {activeDebts.length} outstanding
          </p>
        </div>
        {!isAddingDebt && (
          <button onClick={() => setIsAddingDebt(true)} className="btn-primary inline-flex items-center gap-1.5">
            <Plus size={13} />
            Add debt
          </button>
        )}
      ...
      {isAddingDebt && (
        <form onSubmit={handleCreateDebt} className="card p-6 space-y-4">
          <div className="flex justify-between items-center">
            <p className="eyebrow">Register liability</p>
            <button
              type="button"
              onClick={() => {
                setIsAddingDebt(false);
          ...
          <div>
            ...
            />
            {errors.source && (
              <span className="mono text-[11px] mt-1 block" style={{ color: 'var(--danger)' }}>
                {errors.source}
              </span>
            )}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="eyebrow block mb-2">Principal ({currency})</label>
              <input
                ref={amountInputRef}
                type="number"
                placeholder="0.00"
            ...
            <div>
              <label className="eyebrow block mb-2">Target account</label>
              <select
          ...
          <div>
            <label className="eyebrow block mb-2">Due date</label>
            <input
              type="date"
              className="input"
              value={dueDate}
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="Zero-interest plan..."
              value={notes}
            ...
            .map((debt) => {
              ...
              return (
                <div
                  ...
                  data-debt-status={isFullyPaid ? 'paid' : 'outstanding'}
                  className="card p-5 space-y-4 overflow-hidden relative"
                >
                  ...
                  <div className="flex flex-col sm:flex-row justify-between gap-3">
                    <div>
                      ...
                      <p
                        className="mono text-[11px] mt-1 inline-flex items-center gap-1.5"
                    ...
                    <div className="text-left sm:text-right">
                      ...
                      {deleteConfirmId === debt.id ? (
                        <span className="inline-flex gap-1 mt-2">
                          ...
                          <button onClick={() => setDeleteConfirmId(null)} className="btn-ghost text-[12px]">
                            Cancel
                          </button>
                        </span>
                      ) : (
                        <button
                          onClick={() => {
                            ...
                            setTimeout(() => setDeleteConfirmId((c) => (c === debt.id ? null : c)), 5000);
                          }}
                          className="mono text-[11px] underline mt-1 px-2 py-1 inline-block"
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between mono text-[11px]">
                      ...
                      <span className="font-bold">{payoffPct}% settled</span>
                    </div>
                    <div className="h-2 w-full rounded-full bg-[var(--surface-3)] overflow-hidden">
                      <div className="h-full mw-progress rounded-full" style={{ width: `${payoffPct}%` }} />
                    </div>
                    <div className="flex justify-between mono text-[10px]" style={{ color: 'var(--ink-3)' }}>
                      <span>
                        Cleared {currency} {repaid.toLocaleString()}
                      </span>
                      <span>
                        Principal {currency} {debt.totalAmount.toLocaleString()}
                      </span>
                    </div>
                  </div>

                  <p className="text-[12px] italic leading-relaxed p-3 card-flat" style={{ color: 'var(--ink-2)' }}>
                    "{debt.notes}"
                  </p>
                  ...
                  {!isFullyPaid && payingDebtId !== debt.id && increasingDebtId !== debt.id && (
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button onClick={() => setIncreasingDebtId(debt.id)} className="btn-ghost flex-1 text-[12px]">
                        Add more
                      </button>
                      <button
                        onClick={() => setDetailDebt(debt)}
                        className="btn-ghost inline-flex items-center justify-center gap-1.5 !rounded-full text-[12px] px-4"
                      >
                        <Eye size={13} />
                        Details
                      </button>
                  ...
                  {(isFullyPaid || payingDebtId === debt.id || increasingDebtId === debt.id) && (
                    <div className="flex justify-end">
                      <button
                        onClick={() => setDetailDebt(debt)}
                        className="btn-ghost inline-flex items-center gap-1.5 !rounded-full text-[11px] px-3 py-1.5"
                      >
                        <Eye size={12} />
                        Details
                      </button>
                  ...
                  {increasingDebtId === debt.id && (
                    <form onSubmit={handleIncreaseDebtSubmit} className="card-flat p-4 space-y-3">
                      <div className="flex justify-between items-center">
                        <p className="eyebrow">Increase debt</p>
                        <button
                          type="button"
                          onClick={() => setIncreasingDebtId(null)}
                          className="btn-ghost text-[12px]"
                        >
                          Cancel
                        </button>
                      ...
                      <div>
                        ...
                        <select
                          value={
                            incTargetAccountId && incTargetAccountType
                              ? `${incTargetAccountId}:${incTargetAccountType}`
                              : 'other'
                          }
                          onChange={(e) => {
                  ...
                  {payingDebtId === debt.id && (
                    <form onSubmit={handlePayDebtSubmit} className="card-flat p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="eyebrow block mb-2">Deduct from</label>
                          <select
                      ...
                      {paySourceType === 'card' && paySourceId && (
                        <div className="card-flat !p-3 space-y-2">
                          <label className="eyebrow block">Card charge ({currency})</label>
                          <input
                            type="number"
                            step="any"
                            placeholder="0"
                          ...
                          <p className="mono text-[11px]" style={{ color: 'var(--ink-3)' }}>
                            Optional bank fee deducted from card balance.
                          </p>
                        </div>
                      )}
                      {paymentError && (
                        <p
                          className="mono text-[11px] p-2 rounded-xl flex flex-wrap gap-1.5 min-w-0 break-words"
                          style={{
                            color: 'var(--danger)',
      ...
      {detailDebt && (
        <DebtDetailModal
          ...
          onIncreaseDebt={(id, amt, accId, accType) => {
            onIncreaseDebt(id, amt, accId, accType);
          }}
        />
      )}
