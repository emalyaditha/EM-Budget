...
export default function InflowsOutflows({
  ...
  const validateIncome = (source: string, amtStr: string, target: string, sub: boolean) => {
    ...
    if (sub || source) {
      if (!source.trim()) errs.source = 'Source required';
      else if (source.trim().length < 3) errs.source = 'At least 3 chars';
      else if (/[<>{}]/.test(source)) errs.source = 'Invalid chars';
    }
    if (sub || amtStr) {
    ...
    if (sub || target) {
      if (!target) errs.target = 'Target required';
      else {
        const [id, type] = target.split(':');
        if (type === 'card') {
          const m = cards.find((c) => c.id === id);
          if (m && m.isFrozen) errs.target = 'Card is FROZEN';
        }
      }
    }
    setIncErrors(errs);
  ...
  const validateExpense = (
    ...
    amtStr: string,
    methodId: string,
    methodType: 'cash' | 'card',
    sub: boolean,
  ) => {
    const errs: Record<string, string> = {};
    if (sub || title) {
      if (!title.trim()) errs.title = 'Title required';
      else if (title.trim().length < 3) errs.title = 'At least 3 chars';
      else if (/[<>{}]/.test(title)) errs.title = 'Invalid chars';
    }
    if (sub || amtStr) {
      if (!amtStr) errs.amount = 'Amount required';
      else {
        const n = parseFloat(amtStr);
        const charge = expMethodType === 'card' ? parseFloat(expBankCharge) || 0 : 0;
        if (isNaN(n)) errs.amount = 'Must be a number';
        else if (n <= 0) errs.amount = 'Must be positive';
        else if (methodId) {
          let avail = 0;
          if (methodType === 'cash') {
            const m = cashAccounts.find((c) => c.id === methodId);
            avail = m ? m.balance : 0;
          } else {
            const m = cards.find((c) => c.id === methodId);
            if (m) avail = m.cardType === 'Credit' ? (m.limit ?? 0) + m.currentBalance : m.currentBalance;
          }
          if (avail < n + charge)
            errs.amount = `Insufficient — need ${currency} ${(n + charge).toLocaleString()}, avail ${currency} ${avail.toLocaleString()}`;
        }
      }
    }
    if (sub || methodId) {
      if (!methodId) errs.methodId = 'Source required';
      else if (methodType === 'card') {
        const m = cards.find((c) => c.id === methodId);
        if (m && m.isFrozen) errs.methodId = 'Card is FROZEN';
      }
    }
    setExpErrors(errs);
    return Object.keys(errs).length === 0;
  ...
  const handleIncomeSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateIncome(incSource, incAmount, targetComp, true)) {
      if (!incSource.trim()) incSourceRef.current?.focus();
      else if (!incAmount) incAmountRef.current?.focus();
      else incTargetRef.current?.focus();
      showToast('error', 'Fix inflow errors.');
      return;
    }
    setIsProcessing(true);
    try {
      await onAddIncome(parseFloat(incAmount), incDate, incSource || 'Inflow', incCategory, incTargetId, incTargetType);
      setIncAmount('');
      setIncSource('');
      setIncCategory('Salary');
      setIncSubmitted(false);
      setIncErrors({});
      showToast('success', 'Inflow recorded.');
    } catch {
  ...
  const handleExpenseSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateExpense(expTitle, expDesc, expAmount, expMethodId, expMethodType, true)) {
      if (!expTitle.trim()) expTitleRef.current?.focus();
      else if (!expAmount) expAmountRef.current?.focus();
      else expTargetRef.current?.focus();
      showToast('error', 'Fix outflow errors.');
      return;
    }
    setIsProcessing(true);
    try {
      ...
      setExpErrors({});
      showToast('success', 'Outflow settled.');
    } catch {
  ...
  const handleSelectTargetAccount = (value: string) => {
    ...
    if (incSubmitted) validateIncome(incSource, incAmount, value, true);
  };
  const handleSelectPaymentMethod = (value: string) => {
    ...
    setInsufficiencyError(null);
    if (expSubmitted) validateExpense(expTitle, expDesc, expAmount, id, type as 'cash' | 'card', true);
  };
  const handleScanSuccess = (data: {
    ...
    if (data.transactionType === 'income') {
      ...
      const cats: CategoryIncome[] = ['Salary', 'Freelance', 'Business', 'Bonus', 'Commission', 'Other'];
      setIncCategory(cats.find((c) => c.toLowerCase() === data.category.toLowerCase()) || 'Other');
      if (data.date) setIncDate(data.date);
    } else {
      setToggleForm('expense');
      setExpTitle(data.title || '');
      setExpDesc(data.description || '');
      setExpAmount(data.amount ? data.amount.toString() : '');
      const cats: CategoryExpense[] = [
        'Food',
        'Transport',
        'Shopping',
        'Utilities',
      ...
      setExpCategory(cats.find((c) => c.toLowerCase() === data.category.toLowerCase()) || 'Other');
      if (data.date) setExpDate(data.date);
      if (data.bankCharge) setExpBankCharge(data.bankCharge.toString());
    }
  };
    ...
    <div id="inflows-outflows-view" className="space-y-5">
      ...
      <div className="flex gap-2 p-1.5 rounded-full border border-[var(--line)] bg-[var(--surface)] w-fit shadow-sm">
        <button
          ...
          className={`pill !py-2 !px-5 text-xs font-bold flex items-center gap-1.5 border-transparent ${toggleForm === 'income' ? 'pill-active !border-[var(--ink)]' : '!bg-transparent hover:!bg-[var(--surface-2)]'}`}
        >
          <PlusCircle size={14} />
          Inflow
        </button>
        <button
          ...
          className={`pill !py-2 !px-5 text-xs font-bold flex items-center gap-1.5 border-transparent ${toggleForm === 'expense' ? 'pill-active !border-[var(--ink)]' : '!bg-transparent hover:!bg-[var(--surface-2)]'}`}
        >
          <MinusCircle size={14} />
          Outflow
        </button>
      ...
      <div className="card p-5 md:p-6 relative overflow-hidden">
        ...
        {toggleForm === 'income' ? (
          <form onSubmit={handleIncomeSubmit} className="space-y-5" id="log-income-form">
            <div>
              <span className="eyebrow">Capture inflow</span>
              <p className="text-xs text-[var(--ink-2)] mt-1">Record money in — salary, freelance, business.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">Destination</label>
                <select
                  ref={incTargetRef}
          ...
          <form onSubmit={handleExpenseSubmit} className="space-y-5" id="log-expense-form">
            <div>
              <span className="eyebrow">Settle outflow</span>
              <p className="text-xs text-[var(--ink-2)] mt-1">Log money out — bills, shopping, rent.</p>
            </div>
            <div className="ledger-rule" />
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="input"
              />
              {expErrors.title && <span className="mono text-[11px] text-[var(--danger)]">{expErrors.title}</span>}
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="eyebrow normal-case">
                Remarks <span className="text-[var(--ink-3)] font-normal">(optional)</span>
              </label>
              <input
                type="text"
                placeholder="Ref #8291"
                value={expDesc}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">Deduct from</label>
                <select
                  ref={expTargetRef}
                ...
                {expErrors.methodId && (
                  <span className="mono text-[11px] text-[var(--danger)]">{expErrors.methodId}</span>
                )}
            ...
            >
              <MinusCircle size={14} />
              {isProcessing ? 'Processing…' : 'Settle outflow'}
            </button>
          </form>
        )}
      </div>
