...
import type { LoanGiven, CashAccount, BankCard } from '../types';
import {
  Plus,
  CheckCircle2,
  Calendar,
...
export default function LoansTracker({
  ...
  const availableAccounts = useMemo(
    () => [
      ...cashAccounts.map((acc) => ({
        id: acc.id,
        name: `${acc.name} (Wallet)`,
        balance: acc.balance,
        type: 'cash' as const,
      })),
      ...cards.map((c) => ({
        id: c.id,
        name: `${c.bankName} - ${c.cardName} (${c.cardType})`,
  ...
  const handleGiveLoanSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateLoanForm()) {
      showToast('error', 'Resolve errors.');
      return;
    }
    ...
    setErrors({});
    showToast('success', 'Loan recorded.');
  };
  ...
  const handleSettleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(amt) || amt <= 0) {
      setSettlementError('Amount must be > 0.');
      return;
    }
    if (amt > currentLoan.remainingAmount) {
      setSettlementError(`Exceeds remaining ${currency} ${currentLoan.remainingAmount.toLocaleString()}.`);
      return;
    }
    ...
    showToast('success', `Settlement ${currency} ${amt.toLocaleString()} credited.`);
  };
  ...
  const handleIncreaseSubmit = (e: React.FormEvent, loan: LoanGiven) => {
    ...
    if (isNaN(amt) || amt <= 0) {
      setIncreaseError('Amount must be > 0.');
      return;
    }
    ...
    if (selectedAcc && selectedAcc.balance < amt + chargeVal) {
      setIncreaseError(
        `Insufficient ${currency} ${selectedAcc.balance.toLocaleString()} for ${currency} ${(amt + chargeVal).toLocaleString()}.`,
      );
      return;
    }
    onIncreaseLoan(
      ...
      increaseSourceType,
      sourceName,
      increaseNotes.trim() || 'Additional capital',
      chargeVal,
    );
    setIncreaseAmount('');
  ...
  const handleDeleteLoanClick = (loanId: string, name: string) => {
    showConfirm({
      message: `Remove loan tracker for "${name}"?`,
      onConfirm: () => {
        onDeleteLoan(loanId);
        showToast('info', 'Loan tracker removed.');
      },
    });
  };
      ...
      <div className="gradient-card p-6 overflow-hidden" style={{ background: 'var(--gradient-card-dark)' }}>
        <div className="flex flex-col sm:flex-row justify-between gap-4 relative z-10">
          <div>
            ...
            <h2 className="text-[22px] font-bold tracking-tight mt-1 text-white">Loans given</h2>
            <p className="text-[13px] mt-1 text-white/60">Capital lent · settlements · ledger history.</p>
          </div>
          <button
            onClick={() => setIsGivingLoan(!isGivingLoan)}
            className="pill pill-active self-start sm:self-center inline-flex items-center gap-1.5 !bg-[var(--accent)] !text-[var(--accent-fg)] !border-[var(--accent)]"
          >
            <Plus size={13} />
            {isGivingLoan ? 'Close form' : 'Lend & record'}
          </button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5 relative z-10">
          <div className="rounded-[14px] p-3 bg-white/10 border border-white/10">
            <p className="eyebrow !text-white/60 !text-[9px]">Outstanding</p>
            <p className="mono text-[15px] font-bold mt-1 text-white">
              {currency} {totalRemainingAmount.toLocaleString()}
            </p>
          </div>
          <div className="rounded-[14px] p-3 bg-white/10 border border-white/10">
            <p className="eyebrow !text-white/60 !text-[9px]">Total lent</p>
            <p className="mono text-[15px] font-bold mt-1 text-white">
              {currency} {totalLentAmount.toLocaleString()}
            </p>
          </div>
          <div className="rounded-[14px] p-3 bg-white/10 border border-white/10">
            <p className="eyebrow !text-white/60 !text-[9px]">Recovered</p>
            <p className="mono text-[15px] font-bold mt-1 text-white">
              {currency} {totalRecoveredAmount.toLocaleString()}
            </p>
          </div>
        </div>
      ...
      {isGivingLoan && (
        <form onSubmit={handleGiveLoanSubmit} className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="eyebrow block mb-2">Borrower</label>
              <input
                type="text"
              ...
              />
              {errors.borrowerName && (
                <p className="mono text-[11px] mt-1" style={{ color: 'var(--danger)' }}>
                  {errors.borrowerName}
                </p>
              )}
            </div>
            ...
            <div>
              <label className="eyebrow block mb-2">Debit from</label>
              <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="eyebrow block mb-2">Date lent</label>
              <input type="date" className="input" value={dateGiven} onChange={(e) => setDateGiven(e.target.value)} />
              {errors.dateGiven && (
                <p className="mono text-[11px] mt-1" style={{ color: 'var(--danger)' }}>
                  {errors.dateGiven}
                </p>
        ...
        {activeLoans.length === 0 ? (
          <div className="empty">
            ...
            <p className="text-[12px] mt-1" style={{ color: 'var(--ink-2)' }}>
              Lend & record to start tracking receivables.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  >
                    <div>
                      <h3 className="text-[14px] font-bold flex items-center gap-2 min-w-0">
                        ...
                        <span
                          className="mono text-[9px] font-bold px-2 py-0.5 rounded-full shrink-0"
                      ...
                      <p
                        className="mono text-[11px] mt-1 flex flex-wrap gap-1 items-center min-w-0"
                        style={{ color: 'var(--ink-2)' }}
                      >
                        <Calendar size={11} className="shrink-0" />
                        <span className="truncate">
                          {loan.dateGiven} · {loan.sourceAccountName}
                        </span>
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <p className="eyebrow !text-[9px]">Outstanding</p>
                      <p className="mono text-[15px] font-bold mt-1">
                        {currency} {loan.remainingAmount.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <p className="eyebrow !text-[9px]">Capital lent</p>
                      <p className="mono text-[13px] font-bold mt-1">
                        {currency} {loan.totalAmount.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <p className="eyebrow !text-[9px]">Progress</p>
                      <div className="flex justify-between mono text-[11px] mt-1">
                        <span style={{ color: 'var(--ink-2)' }}>{activeProgress}% settled</span>
                      </div>
                      <div className="h-2 w-full mt-2 rounded-full bg-[var(--surface-3)] overflow-hidden">
                        <div className="h-full mw-progress rounded-full" style={{ width: `${activeProgress}%` }} />
                      </div>
                    </div>
                  </div>

                  {loan.notes && (
                    <p className="text-[12px] italic p-3 card-flat break-words" style={{ color: 'var(--ink-2)' }}>
                      "{loan.notes}"
                    </p>
                  )}
                  ...
                  {settlingLoanId === loan.id && (
                    <form onSubmit={handleSettleSubmit} className="card-flat p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div>
                          <label className="eyebrow block mb-2">Credit to</label>
                          <select
                      ...
                      {receivedInType === 'card' && receivedInId && (
                        <div className="card-flat !p-3">
                          <label className="eyebrow block mb-2">Card charge ({currency})</label>
                          <input
                            type="number"
                            step="any"
                            placeholder="0"
                      ...
                      {settlementError && (
                        <p className="mono text-[11px]" style={{ color: 'var(--danger)' }}>
                          {settlementError}
                        </p>
                      )}
                    </form>
                  ...
                  {increasingLoanId === loan.id && (
                    <form
                      onSubmit={(e) => handleIncreaseSubmit(e, loan)}
                      className="card-flat p-4 space-y-3"
                      id={`lend-more-form-${loan.id}`}
                    >
                      <p className="eyebrow inline-flex items-center gap-1">
                  ...
                  <div>
                    ...
                    >
                      <History size={11} />
                      History ({loan.settlements?.length || 0}){' '}
                    ...
                    {expandedLoanId === loan.id && (
                      <div className="mt-2 divide-y" style={{ borderTop: '1px solid var(--line)' }}>
                        {!loan.settlements || loan.settlements.length === 0 ? (
                          <p className="mono text-[11px] py-3" style={{ color: 'var(--ink-3)' }}>
                            No settlements yet.
                          </p>
                        ) : (
                          loan.settlements.map((s) => (
                            <div key={s.id} className="flex justify-between items-center py-2.5">
                              <div>
                                <p
                                  className="mono text-[11px] font-semibold inline-flex items-center gap-1"
                                  style={{ color: 'var(--ink-2)' }}
                                >
                                  <ArrowDownLeft size={11} />
                                  Repaid · {s.receivedInName}
                                </p>
