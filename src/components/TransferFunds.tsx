  ...
  const validateTransfer = (from: string, to: string, amtStr: string, chargeStr: string, sub: boolean) => {
    ...
    if (from && !errs.amount && !errs.charge) {
      ...
      if (src) {
        ...
        if (total > bal)
          errs.amount = `Total ${currency} ${total.toLocaleString()} exceeds ${currency} ${bal.toLocaleString()}`;
      }
    }
    setErrors(errs);
        ...
        <div className="flex items-start justify-between gap-4 relative z-10">
          <div>
            ...
            <h3 className="text-[16px] font-bold tracking-tight mt-1 flex items-center gap-2 text-white">
              <span className="w-7 h-7 rounded-full bg-[var(--accent)] text-[var(--accent-fg)] grid place-items-center">
          ...
          <span className="hidden sm:inline-flex pill !bg-white/10 !border-white/15 !text-white mono !text-[10px] tracking-widest">
            INSTANT
          </span>
        </div>
      ...
      <div className="card p-6 md:p-7 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_48px_1fr] items-end gap-3">
          <div className="space-y-1.5">
            ...
            <select
              ...
              className={`input ${errors.from ? '!border-[var(--danger)]' : ''}`}
            >
          ...
          <div className="hidden md:grid place-items-center pb-1">
            <div className="w-9 h-9 rounded-full border border-[var(--line)] bg-[var(--surface-2)] grid place-items-center">
              <ArrowRightLeft
                size={14}
                className={fromAccount && toAccount ? 'text-[var(--ink)]' : 'text-[var(--ink-3)]'}
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className={`input ${errors.to ? '!border-[var(--danger)]' : ''}`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className={`input mono ${errors.amount ? '!border-[var(--danger)]' : ''}`}
              required
            />
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className={`input mono ${errors.charge ? '!border-[var(--danger)]' : ''}`}
            />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              placeholder="e.g. Move reserves to card"
              value={note}
        ...
        <button
          type="submit"
          className="w-full h-[44px] gap-2 rounded-full font-bold text-[13px] text-white flex items-center justify-center"
          style={{ background: 'var(--gradient-card-orange)' }}
        >
          <ArrowRightLeft size={14} /> Execute transfer
        </button>
        <p className="mono text-[11px] text-[var(--ink-3)] text-center">
          Balances update instantly. No hidden spread — like Wise.
        </p>
      </div>
    </form>
  );
