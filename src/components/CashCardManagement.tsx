...
import type { CashAccount, BankCard, Charge } from '../types';
import {
  Plus,
  Trash2,
  Edit,
        ...
        <div>
          ...
          <span className="mono text-[20px] font-bold tracking-tight text-[var(--ink)] leading-none mt-0.5 block tabular-nums">
            ...
            {(card.cardType === 'Credit'
              ...
              : card.currentBalance - (card.lockedAmount ?? 0)
...
export default function CashCardManagement({
  ...
  const [chargeType, setChargeType] = useState<'Interest' | 'LatePayment' | 'OverLimit' | 'Annual' | 'Custom'>(
    'Interest',
  );
  ...
  const validateCash = (name: string, balance: string, submitted: boolean) => {
    ...
    if (submitted || balance) {
      if (balance === '') errs.balance = 'Starting amount is required';
        ...
        if (isNaN(n)) errs.balance = 'Must be a number';
        else if (n < 0) errs.balance = 'Cannot be negative';
      }
    }
    setCashErrors(errs);
  ...
  const validateCard = (name: string, bank: string, balance: string, numStr: string, submitted: boolean) => {
    ...
    if (submitted || balance) {
      if (balance === '') errs.balance = 'Starting balance is required';
        ...
        if (isNaN(amt)) errs.balance = 'Must be a number';
        else if (amt < 0) errs.balance = 'Cannot be negative';
      }
    }
    if (numStr) {
      const c = numStr.replace(/\s+/g, '');
      if (c && !/^\d+$/.test(c)) errs.number = 'Digits only';
      else if (c && (c.length < 8 || c.length > 19)) errs.number = '8–19 digits';
    }
    setCardErrors(errs);
    return Object.keys(errs).length === 0;
  };
  ...
  const handleCreateCash = (e: React.FormEvent) => {
    ...
    if (!ok) {
      if (!cashName.trim()) cashNameInputRef.current?.focus();
      else cashBalanceInputRef.current?.focus();
      showToast('error', 'Fix wallet errors.');
      return;
    }
    ...
    setCashErrors({});
    showToast('success', 'Cash account added.');
  };
  const handleCreateCard = (e: React.FormEvent) => {
    ...
    if (!ok) {
      if (!cardName.trim()) cardNameInputRef.current?.focus();
      else if (!bankName.trim()) bankNameInputRef.current?.focus();
      else if (!cardBalance) cardBalanceInputRef.current?.focus();
      else cardNumberInputRef.current?.focus();
      showToast('error', 'Fix card errors.');
      return;
    }
    const bal = parseFloat(cardBalance) || 0;
    const lim = cardType === 'Credit' ? parseFloat(cardLimit) || 0 : undefined;
    let clean = cardNumber.replace(/\s+/g, '');
    if (clean.length > 0) {
    ...
    onAddCard({
      ...
      cardType,
      currentBalance: cardType === 'Credit' ? -Math.abs(bal) : bal,
      limit: lim,
      cardNumber: clean,
      cardTheme,
      lockedAmount: cardType === 'Debit' ? parseFloat(cardLockedAmount) || 0 : undefined,
      dueDate: cardType === 'Credit' && cardDueDate ? cardDueDate : undefined,
      statementCloseDate: cardType === 'Credit' && cardStatementCloseDate ? cardStatementCloseDate : undefined,
  ...
  const validateEditCard = (name: string, numStr: string) => {
    const errs: Record<string, string> = {};
    if (!name.trim()) errs.name = 'Card name required';
    else if (name.trim().length < 3) errs.name = 'At least 3 chars';
    else if (/[<>{}]/.test(name)) errs.name = 'Invalid chars';
    const c = numStr.replace(/\s+/g, '').replace(/\*/g, '');
    if (c && c.length > 0 && !/^\d+$/.test(c)) errs.number = 'Digits only';
  ...
  const handleSaveEditCard = (e: React.FormEvent) => {
    ...
    let clean = editCardNumber.replace(/\s+/g, '').replace(/\*/g, '');
    if (clean.length > 0) clean = clean.length > 4 ? `•••• •••• •••• ${clean.slice(-4)}` : `•••• •••• •••• ${clean}`;
    else clean = editingCard.cardNumber || `•••• •••• •••• ${Math.floor(1000 + Math.random() * 9000)}`;
    onUpdateCard({
      ...editingCard,
      ...
      statementCloseDate:
        editingCard.cardType === 'Credit' && editCardStatementCloseDate
          ? editCardStatementCloseDate
          : editingCard.statementCloseDate,
      apr: editingCard.cardType === 'Credit' && editCardApr ? parseFloat(editCardApr) : editingCard.apr,
      minPayment:
        editingCard.cardType === 'Credit' && editCardMinPayment
          ? parseFloat(editCardMinPayment)
          : editingCard.minPayment,
    });
    setEditingCard(null);
  ...
  const handleQuickAdjustCash = (e: React.FormEvent) => {
    ...
    const acc = cashAccounts.find((c) => c.id === selectedCashId);
    if (!acc) return;
    if (!validateQuick(qtyAction, true)) {
      qtyActionInputRef.current?.focus();
      showToast('error', 'Check amount');
      return;
    }
    ...
    <div id="cash-card-vault-view" className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 relative z-10">
          <div>
            ...
            <p className="text-[13px] text-white/60 mt-1">
              {cashAccounts.length} {cashAccounts.length === 1 ? 'account' : 'accounts'} • Physical holdings
            </p>
          </div>
          <span className="pill !bg-white/10 !border-white/15 !text-white mono !text-[10px] tracking-widest">
            ● Live ledger
          </span>
        </div>
      ...
      <section className="card p-5 md:p-7">
        <div className="flex items-center gap-3 pb-4 border-b border-[var(--line)]">
          <div className="w-8 h-8 rounded-full border border-[var(--line)] bg-[var(--surface-2)] flex items-center justify-center text-[var(--ink-2)]">
            <Wallet size={14} />
          </div>
          <div>
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mono text-[15px] font-bold tabular-nums text-[var(--ink)] tracking-tight">
                      {currency}
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full border border-[var(--line)] bg-[var(--surface)] flex items-center justify-center text-[var(--ink-3)] hover:text-[var(--danger)] hover:border-[var(--danger)]/30 transition-colors"
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
        ...
        {selectedCashId && actionType && (
          <form
            onSubmit={handleQuickAdjustCash}
            className="mt-4 p-4 rounded-[16px] border border-[var(--line)] bg-[var(--surface-2)] flex flex-col gap-3"
          >
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="eyebrow normal-case tracking-normal !text-[11px] hover:text-[var(--ink)]"
              >
                Cancel
              </button>
            </div>
        ...
        <form onSubmit={handleCreateCash} className="mt-6 pt-5 border-t border-[var(--line)] flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {cashErrors.balance && (
                <span className="text-[11px] text-[var(--danger)] mono">{cashErrors.balance}</span>
              )}
            </div>
          </div>
          <button type="submit" className="btn-primary w-full flex items-center justify-center gap-2">
            <Plus size={14} />
            Add holdings account
          </button>
        </form>
      </section>
      ...
      <section className="card p-5 md:p-7">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[var(--line)]">
          ...
          {!isAddingCard && (
            <button onClick={() => setIsAddingCard(true)} className="btn-ghost !text-xs flex items-center gap-1.5">
              <Plus size={13} />
              New Bank Card
            </button>
          )}
        ...
        {isAddingCard && (
          <form
            ...
            className="mt-5 p-5 rounded-[16px] border border-[var(--line)] bg-[var(--surface-2)] space-y-4"
          >
            <div className="flex justify-between items-center pb-3 border-b border-[var(--line)]">
              ...
              <button
                ...
                className="eyebrow normal-case tracking-normal hover:text-[var(--ink)]"
              >
                Cancel
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">Card nickname</label>
                <input
                  ...
                  className="input"
                />
                {cardErrors.name && <span className="text-[11px] text-[var(--danger)] mono">{cardErrors.name}</span>}
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">Bank issuer</label>
                <input
                  ref={bankNameInputRef}
                  type="text"
                  placeholder="HNB Bank"
                  value={bankName}
                  onChange={(e) => {
                  ...
                  className="input"
                />
                {cardErrors.bank && <span className="text-[11px] text-[var(--danger)] mono">{cardErrors.bank}</span>}
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">Card type</label>
                <select
                  value={cardType}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">
                  {cardType === 'Credit' ? 'Starting debt (' + currency + ')' : 'Starting balance (' + currency + ')'}
                </label>
                <input
                ...
                />
                {cardErrors.balance && (
                  <span className="text-[11px] text-[var(--danger)] mono">{cardErrors.balance}</span>
                )}
              </div>
            </div>
            {cardType === 'Credit' && (
              <div className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="flex flex-col gap-1.5">
                    <label className="eyebrow normal-case">Credit limit ({currency})</label>
                    <input
                      type="number"
                      placeholder="50000"
                      value={cardLimit}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="eyebrow normal-case">APR (%)</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="eyebrow normal-case">Min. payment ({currency})</label>
                    <input
                      type="number"
                      min="0"
                      placeholder="2500"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1.5">
                    <label className="eyebrow normal-case">Payment due date</label>
                    <input
                      type="date"
                      className="input"
                      value={cardDueDate}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="eyebrow normal-case">Payment cut-off date</label>
                    <input
                      type="date"
                      className="input"
                      value={cardStatementCloseDate}
                      onChange={(e) => setCardStatementCloseDate(e.target.value)}
                    />
                  </div>
                </div>
              </div>
            ...
            {cardType === 'Debit' && (
              <div className="flex flex-col gap-1.5">
                <label className="eyebrow normal-case">Locked amount ({currency})</label>
                <input
                  type="number"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="eyebrow normal-case">Card number (optional)</label>
              <input
                ref={cardNumberInputRef}
                type="text"
                placeholder="4201 9283"
                value={cardNumber}
                onChange={(e) => {
                ...
                className="input mono"
              />
              {cardErrors.number && <span className="text-[11px] text-[var(--danger)] mono">{cardErrors.number}</span>}
            </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {themeOptions.map((th) => (
                  <button
                    ...
                    className={`w-7 h-7 rounded-full border ${th.color} ${cardTheme === th.name ? 'ring-2 ring-offset-2 ring-[var(--ink)] ring-offset-[var(--surface-2)] scale-110' : 'opacity-70 hover:opacity-100'} transition-all border-[var(--line)]`}
                    title={th.name}
                  />
                ))}
              </div>
        ...
        <div className="mt-5 space-y-4">
          {(() => {
            const active = cards.filter((c) => {
              const snakeCanceled = Boolean((c as unknown as Record<string, unknown>).is_canceled);
              return !c.isCanceled && !snakeCanceled;
            });
            const canceled = cards.filter(
              (c) => c.isCanceled || Boolean((c as unknown as Record<string, unknown>).is_canceled),
            );
            return (
              <>
                {active.length === 0 ? (
                  <div className="empty">
                    <p className="text-sm font-medium">No active cards</p>
                    <p className="text-xs mt-1 text-[var(--ink-3)]">Add a debit or credit card.</p>
                  </div>
                ) : (
                  active.map((card, idx) => {
                    const isCredit = card.cardType === 'Credit';
                    const hasNeg = card.currentBalance < 0;
                    const out = hasNeg ? Math.abs(card.currentBalance) : 0;
                    const lim = card.limit ?? 0;
                    const lockedAmt = card.lockedAmount ?? 0;
                    const avail = lim + card.currentBalance;
                    const used = lim > 0 ? Math.max(0, lim - avail) : 0;
                    const pct = lim > 0 ? Math.min(100, (used / lim) * 100) : 0;
                    const hasDetails = isCredit || (card.lockedAmount !== undefined && card.lockedAmount > 0);
                    const isExp = !!expandedCardIds[card.id];
                    return (
                      <div key={card.id} className="space-y-2">
                        <InteractiveBankCard
                          ...
                          setEditCardStatementCloseDate={setEditCardStatementCloseDate}
                          onClick={() => {
                        ...
                        {hasDetails && (
                          <div className="flex justify-center">
                            <button
                              type="button"
                              onClick={() => setExpandedCardIds((p) => ({ ...p, [card.id]: !p[card.id] }))}
                              className="btn-ghost !px-3 !py-1 !text-[11px] flex items-center gap-1"
                            >
                              <span>{isExp ? 'Hide' : 'Show'} details</span>
                        ...
                        {hasDetails && (
                          <div
                            className={`overflow-hidden transition-all duration-300 ${isExp ? 'max-h-[520px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}
                          >
                                ...
                                <>
                                  ...
                                  <div className="w-full h-1.5 bg-[var(--surface-2)] border border-[var(--line)] rounded-full overflow-hidden">
                                    <div
                                      ...
                                      style={{
                                        width: `${card.currentBalance > 0 ? Math.max(0, Math.min(100, ((card.currentBalance - lockedAmt) / card.currentBalance) * 100)) : 0}%`,
                                      }}
                                    />
                                  </div>
                              ...
                              {isCredit && (
                                <>
                                  ...
                                  {lim > 0 && (
                                    <>
                                      <div className="flex justify-between items-center">
                                        <span className="eyebrow normal-case">Utilization</span>
                                        <span className="mono text-xs font-bold">{pct.toFixed(0)}% used</span>
                                      </div>
                                      <div className="w-full h-1.5 bg-[var(--surface-2)] border border-[var(--line)] rounded-full overflow-hidden">
                ...
                {canceled.length > 0 && (
                  <div className="pt-4 border-t border-[var(--line)]">
                    <button
                      ...
                      className="w-full btn-ghost flex justify-between !rounded-[12px]"
                    >
                      <span className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-[var(--ink-3)]" />
                        Archived ({canceled.length})
                      </span>
                      <span>{showCanceled ? 'Hide' : 'Show'}</span>
                    </button>
                    {showCanceled && (
                      <div className="space-y-3 mt-3">
                        {canceled.map((card, idx) => (
                            ...
                            <div className="absolute top-3 right-3">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  showToast('success', `${card.cardName} reactivated.`);
                                }}
                                className="btn-ghost !px-2.5 !py-1 !text-[11px] flex items-center gap-1"
                              >
                                <RefreshCw size={10} />
      ...
      {editingCard &&
        (() => {
          ...
          const curCharges = editingCard.charges || [];
          return (
            <div
              className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--ink)]/40 backdrop-blur-sm"
              onClick={() => setEditingCard(null)}
            >
              <div
                className={`card p-6 w-full max-h-[90vh] overflow-y-auto ${isCredit ? 'max-w-3xl' : 'max-w-md'}`}
              ...
              >
                <div className="flex justify-between items-center pb-4 border-b border-[var(--line)] mb-5">
                  <div>
                    ...
                    <p className="text-xs text-[var(--ink-2)]">Configure {editingCard.cardName}</p>
                  </div>
                  <button onClick={() => setEditingCard(null)} className="btn-ghost !px-3 !py-1.5 !text-xs">
                    Close
                  </button>
                </div>
                  ...
                  <form onSubmit={handleSaveEditCard} className="space-y-4">
                    ...
                    <div className="card-flat p-4 space-y-3 bg-[var(--surface-2)]">
                      <div className="flex flex-col gap-1.5">
                        ...
                        />
                        {editCardErrors.name && (
                          <span className="text-[11px] text-[var(--danger)] mono">{editCardErrors.name}</span>
                        )}
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label className="eyebrow normal-case">Card number</label>
                        <input
                          type="text"
                      ...
                      {!isCredit && (
                        <div className="flex flex-col gap-1.5">
                          <label className="eyebrow normal-case">Locked ({currency})</label>
                          <input
                            type="number"
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex flex-col gap-1.5">
                              ...
                              <input
                                ...
                                min="0"
                                value={editCardApr}
                                onChange={(e) => setEditCardApr(e.target.value)}
                                className="input mono !text-xs"
                              />
                            </div>
                            <div className="flex flex-col gap-1.5">
                              <label className="eyebrow normal-case">Min. ({currency})</label>
                              <input
                                type="number"
                                min="0"
                            ...
                            <div className="flex flex-col gap-1.5">
                              <label className="eyebrow normal-case">Due date</label>
                              <input
                                type="date"
                                className="input"
                            ...
                            <div className="flex flex-col gap-1.5">
                              <label className="eyebrow normal-case">Cut-off date</label>
                              <input
                                type="date"
                                className="input"
                                value={editCardStatementCloseDate}
                                onChange={(e) => setEditCardStatementCloseDate(e.target.value)}
                              />
                            </div>
                          </div>
                        </>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          {themeOptions.slice(0, 5).map((th) => (
                            <button
                              key={th.name}
                              type="button"
                              onClick={() => setEditCardTheme(th.name)}
                              className={`w-6 h-6 rounded-full border ${th.color} ${editCardTheme === th.name ? 'ring-2 ring-[var(--ink)] ring-offset-2 ring-offset-[var(--surface-2)]' : ''} border-[var(--line)]`}
                            />
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="flex gap-2 justify-end">
                      <button type="button" onClick={() => setEditingCard(null)} className="btn-ghost">
                        Cancel
                      </button>
                      ...
                      <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                        ...
                        {curCharges.length === 0 ? (
                          <div className="empty !py-4">
                            <span className="text-xs">No charges yet.</span>
                          </div>
                        ) : (
                          curCharges.map((ch) => (
                            <div
                              key={ch.id}
                              className="flex justify-between items-center gap-2 p-2.5 rounded-[12px] border border-[var(--line)] bg-[var(--surface)]"
                            >
                              <div className="min-w-0">
                                <div className="flex gap-1.5 items-center">
                                  <span className="text-xs font-bold truncate">{ch.name}</span>
                                  <span className="text-[9px] px-1.5 py-0.5 rounded-full border border-[var(--line)] mono uppercase">
                                    {ch.type.replace('Charge', '').replace('Fee', '').trim()}
                                  </span>
                                </div>
                                <div className="mono text-[10px] text-[var(--ink-3)]">
                                  {ch.appliedDate}
                                  {ch.isRecurring && ` • ${ch.recurringInterval}`}
                                </div>
                              </div>
                              <div className="flex items-center gap-2 shrink-0">
                                <span className="mono text-xs font-bold text-[var(--danger)]">
                                  ...
                                  onClick={() => {
                                    if (onDeleteCardCharge) {
                                      ...
                                      setEditingCard({
                                        ...
                                        charges: curCharges.filter((c) => c.id !== ch.id),
                                      });
                                    }
                                  }}
                                  className="w-6 h-6 rounded-full border border-[var(--line)] flex items-center justify-center hover:text-[var(--danger)]"
                                >
                                  <Trash2 size={11} />
                                </button>
                              </div>
                      ...
                      <div className="card-flat p-3 bg-[var(--surface-2)] space-y-2">
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              const v = e.target.value as
                                'Interest' | 'LatePayment' | 'OverLimit' | 'Annual' | 'Custom';
                              setChargeType(v);
                              setChargeName(CHARGE_DEFAULT_NAMES[v] || 'Custom');
                            }}
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <select
                            ...
                            onChange={(e) =>
                              setChargeRecurring(e.target.value as 'none' | 'Monthly' | 'Yearly' | 'Custom')
                            }
                            className="input !text-xs"
                          >
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (!chargeName || isNaN(amt) || amt <= 0) {
                              showToast('error', 'Valid name & amount required.');
                              return;
                            }
                            const newCharge: Charge = {
                              ...
                              type:
                                chargeType === 'Interest'
                                  ? 'Interest Charge'
                                  : chargeType === 'LatePayment'
                                    ? 'Late Payment Fee'
                                    : chargeType === 'OverLimit'
                                      ? 'Over-Limit Fee'
                                      : chargeType === 'Annual'
                                        ? 'Annual Fee'
                                        : 'Custom Charge',
                              ...
                              recurringInterval: chargeRecurring !== 'none' ? chargeRecurring : undefined,
                            };
