...
export default function DebtDetailModal({
  ...
  const resolveAccountName = (paidFromId: string, paidFromType: 'cash' | 'card'): string => {
    ...
    return (
      cards.find((c) => c.id === paidFromId)?.bankName || cards.find((c) => c.id === paidFromId)?.cardName || 'Card'
    );
  };
  ...
  const handleIncrease = (e: React.FormEvent) => {
    ...
    if (!n || n <= 0) {
      setIncError('Amount must be positive.');
      return;
    }
        ...
        >
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span
                className="pill !py-1 !px-2.5 !text-[10px] mono"
                style={{
                  background: isFullyRepaid ? 'var(--surface-2)' : 'var(--surface)',
                  borderColor: 'var(--line)',
                  color: isFullyRepaid ? 'var(--ink-2)' : 'var(--ink)',
                }}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${isFullyRepaid ? 'bg-emerald-500' : 'bg-amber-500'} animate-pulse`}
                />
                {isFullyRepaid ? 'Fully Repaid' : 'Active'}
              </span>
              <span className="mono text-[11px]" style={{ color: 'var(--ink-3)' }}>
                {debt.payments.length} repayment{debt.payments.length !== 1 ? 's' : ''} ·{' '}
                {debt.increaseHistory?.length || 0} top-up{(debt.increaseHistory?.length || 0) !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div
                className="card-flat p-3 rounded-2xl"
              ...
              <div
                className="card-flat p-3 rounded-2xl flex gap-2 items-start"
                style={{ background: 'var(--surface-2)', border: '1px solid var(--line)' }}
              >
                <Calendar size={12} className="mt-0.5 shrink-0" style={{ color: 'var(--ink-3)' }} />
                <div>
                  <p className="eyebrow !text-[9px]">Due date</p>
                  <p className="mono text-[12px] font-medium mt-1">{debt.dueDate}</p>
                </div>
              </div>
              <div
                className="card-flat p-3 rounded-2xl flex gap-2 items-start"
              ...
              >
                ...
                <div>
                  <p className="eyebrow !text-[9px]">Receipt account</p>
                  <p className="mono text-[12px] font-medium mt-1 truncate">{debt.accountName || 'Other / indirect'}</p>
                </div>
              </div>
            </div>
            {debt.notes && (
              <div
                className="mt-3 p-3 rounded-2xl flex gap-2"
                style={{ background: 'var(--surface-2)', border: '1px solid var(--line)' }}
              >
                <FileText size={12} className="mt-0.5 shrink-0" style={{ color: 'var(--ink-3)' }} />
                <p className="text-[12px] leading-relaxed italic" style={{ color: 'var(--ink-2)' }}>
                  "{debt.notes}"
                </p>
              </div>
            )}
            ...
            <div className="mt-4 space-y-2">
              <div className="flex justify-between mono text-[11px]">
                <span style={{ color: 'var(--ink-2)' }}>Settlement progress</span>
              ...
              <div className="flex justify-between mono text-[10px]" style={{ color: 'var(--ink-3)' }}>
                ...
                <span>
                  {currency} {debt.totalAmount.toLocaleString()} principal
                </span>
              </div>
            </div>
          </div>
                  ...
                  {timeline.map((ev) => (
                    <div key={ev.id} className="relative flex gap-3 items-start">
                      ...
                      <div
                        ...
                        style={{
                          background:
                            ev.type === 'Added' ? 'color-mix(in srgb, #22c55e 14%, var(--surface))' : 'var(--surface)',
                      ...
                      <div
                        className="flex-1 min-w-0 card-flat px-3 py-2.5 rounded-2xl flex justify-between items-center gap-3"
                        style={{ background: 'var(--surface-2)', border: '1px solid var(--line)' }}
                      >
                        <div className="min-w-0">
                          <p className="mono text-[11px] font-bold flex items-center gap-1.5">
                            <span
                              className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-bold tracking-wide ${ev.type === 'Added' ? 'bg-emerald-500 text-white' : 'bg-[var(--ink)] text-[var(--surface)]'}`}
                            >
                              {ev.type}
                            </span>
          ...
          <form
            ...
            className="card-flat p-4 space-y-3 rounded-2xl"
            style={{ background: 'var(--surface-2)', border: '1px solid var(--line)' }}
          >
            ...
            {incError && (
              <p className="mono text-[11px]" style={{ color: 'var(--danger, #ef4444)' }}>
                {incError}
              </p>
            )}
