...
export default function CreditCardManagement({
  ...
  const validatePay = (cardId: string, amtStr: string, srcVal: string, skipAmountCheck = false): boolean => {
    ...
    if (!srcVal) {
      setPayErrors((p) => ({ ...p, [cardId]: 'Funding source required' }));
      return false;
    }
    const source = fundingAccounts.find((a) => `${a.type}-${a.id}` === srcVal);
    if (!source) {
      setPayErrors((p) => ({ ...p, [cardId]: 'Invalid source' }));
      return false;
    }
    if (source.isFrozen) {
      setPayErrors((p) => ({ ...p, [cardId]: 'Source is FROZEN' }));
      return false;
    }
    ...
    if (!skipAmountCheck) {
      if (!amtStr) {
        setPayErrors((p) => ({ ...p, [cardId]: 'Amount required' }));
        return false;
      }
      const amt = parseFloat(amtStr);
      if (isNaN(amt)) {
        setPayErrors((p) => ({ ...p, [cardId]: 'Must be a number' }));
        return false;
      }
      if (amt <= 0) {
        setPayErrors((p) => ({ ...p, [cardId]: 'Must be positive' }));
        return false;
      }
      if (amt > outstanding) {
        setPayErrors((p) => ({ ...p, [cardId]: `Cannot overpay — debt ${currency} ${outstanding.toFixed(2)}` }));
        return false;
      }
      if (amt > sourceBalance) {
        setPayErrors((p) => ({ ...p, [cardId]: `Insufficient — avail ${currency} ${sourceBalance.toFixed(2)}` }));
        return false;
      }
    } else {
      if (outstanding <= 0) {
        setPayErrors((p) => ({ ...p, [cardId]: 'No balance to settle' }));
        return false;
      }
      if (outstanding > sourceBalance) {
        setPayErrors((p) => ({
          ...p,
          [cardId]: `Insufficient for full — avail ${currency} ${sourceBalance.toFixed(2)}`,
        }));
        return false;
      }
    }
  ...
  const validatePurchase = (
    ...
    if (submitted || amtStr) {
      if (!amtStr) errs.amount = 'Amount required';
        ...
        const amt = parseFloat(amtStr);
        if (isNaN(amt)) errs.amount = 'Must be a number';
        else if (amt <= 0) errs.amount = 'Must be positive';
        else if (cardId) {
          const card = creditCards.find((c) => c.id === cardId);
          if (card) {
            const avail = (card.limit ?? 0) + card.currentBalance;
            if (amt > avail) errs.amount = `Exceeds avail ${currency} ${avail.toFixed(2)}`;
          }
        }
      }
    }
    if (submitted || merchant) {
      if (!merchant.trim()) errs.merchant = 'Merchant required';
      else if (merchant.trim().length < 2) errs.merchant = 'At least 2 chars';
      else if (/[<>{}]/.test(merchant)) errs.merchant = 'Invalid chars';
    }
  ...
  const handleAddPurchase = (e: React.FormEvent) => {
    ...
    if (!ok) {
      if (!purCardId) purchaseCardRef.current?.focus();
      else if (!purAmount) purchaseAmountRef.current?.focus();
      else purchaseMerchantRef.current?.focus();
      showToast('error', 'Fix purchase errors.');
      return;
    }
    onAddPurchase({
      cardId: purCardId,
      ...
      description: purDesc,
      merchant: purMerchant,
      date: purDate,
    });
    setPurAmount('');
    setPurDesc('');
    setPurMerchant('');
    setPurDate(todayLocal());
    ...
    <div className="space-y-6" id="credit-cards-vault">
      <div className="gradient-card p-5 md:p-6 overflow-hidden" style={{ background: 'var(--gradient-card-dark)' }}>
        <div className="flex items-center gap-3 pb-4 border-b border-white/10 relative z-10">
          ...
          <span className="ml-auto pill !bg-white/10 !border-white/15 !text-white mono !text-[11px]">
        ...
        <div className="mt-4 space-y-3">
          {creditCards.length === 0 ? (
            <div className="empty">
              <p className="text-sm font-medium">No credit cards on record</p>
              <p className="text-xs mt-1 text-[var(--ink-3)]">Add one in Cash & Cards.</p>
            </div>
          ) : (
            creditCards.map((c) => {
              ...
              return (
                <div key={c.id} className="card p-4 space-y-3 !shadow-none border border-white/10 bg-white/[0.06]">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[var(--line)]">
                    <div>
                      <h4 className="mono text-[13px] font-semibold text-[var(--ink)] flex items-center gap-2">
                        {c.cardName}
                        <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border border-[var(--line)] bg-[var(--surface-2)] text-[var(--ink-2)]">
                          {c.bankName}
                        </span>
                      </h4>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span
                          className={`mono text-[11px] font-bold ${util > 80 ? 'text-[var(--danger)]' : 'text-[var(--ink-2)]'}`}
                        >
                          {util}% used
                        </span>
                        <div className="w-20 h-1 rounded-full bg-[var(--surface-2)] border border-[var(--line)] overflow-hidden hidden sm:block">
                          <div className="h-full bg-[var(--ink)]" style={{ width: `${util}%` }} />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                    <div
                      className={`p-2 rounded-lg border ${isOverdue ? 'border-[var(--danger)] bg-[var(--danger)]/10' : isDueSoon ? 'border-amber-500 bg-amber-500/10' : 'border-[var(--line)] bg-[var(--surface)]'}`}
                    >
                      <div className="flex items-center gap-1 mb-1">
                        <Calendar
                          size={10}
                          className={
                            isOverdue ? 'text-[var(--danger)]' : isDueSoon ? 'text-amber-500' : 'text-[var(--ink-3)]'
                          }
                        />
                        <span className="eyebrow normal-case">{isOverdue ? 'Overdue' : 'Due in'}</span>
                      </div>
                      <span
                        className={`mono font-bold ${isOverdue ? 'text-[var(--danger)]' : isDueSoon ? 'text-amber-500' : 'text-[var(--ink)]'}`}
                      >
                    ...
                    <div
                      className={`p-2 rounded-lg border ${minSatisfied ? 'border-[var(--success)]/40 bg-[var(--success)]/10' : 'border-[var(--line)] bg-[var(--surface)]'}`}
                    >
                      <div className="flex items-center justify-between gap-1 mb-1">
                        <div className="flex items-center gap-1">
                          <AlertTriangle
                            size={10}
                            className={minSatisfied ? 'text-[var(--success)]' : 'text-[var(--ink-3)]'}
                          />
                      ...
                      <span
                        className={`mono font-bold ${minSatisfied ? 'text-[var(--success)]' : 'text-[var(--ink)]'}`}
                      >
                        {currency}
                        {(c.minPayment || 0).toLocaleString()}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        onUpdateCard({ ...c, limit: isNaN(v) ? undefined : v });
                      }}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          onUpdateCard({ ...c, isLimitLocked: true });
                          (e.target as HTMLInputElement).blur();
                          showToast('success', `Limit ${currency}${c.limit || 0} locked.`);
                        }
                      }}
                      className="input !py-1.5 !text-xs mono w-24 disabled:opacity-60"
                      placeholder="Limit"
                    />
                    <button
                      type="button"
                      onClick={() => {
                        const next = !(c.isLimitLocked ?? true);
                        onUpdateCard({ ...c, isLimitLocked: next });
                        showToast(
                          next ? 'success' : 'info',
                          next ? `Locked ${currency}${c.limit || 0}` : 'Unlocked — Enter to save',
                        );
                      }}
                      className="w-7 h-7 rounded-full border border-[var(--line)] bg-[var(--surface)] flex items-center justify-center text-[var(--ink-3)] hover:text-[var(--ink)]"
                    >
                      {(c.isLimitLocked ?? true) ? <Lock size={12} /> : <Unlock size={12} />}
                    </button>
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setPayAmounts((p) => ({ ...p, [c.id]: v }));
                          validatePay(c.id, v, paySources[c.id] || '');
                        }}
                        className="input mono flex-1 !py-2.5 !text-xs"
                      />
                      <select
                        value={paySources[c.id] || ''}
                        onChange={(e) => {
                          const v = e.target.value;
                          setPaySources((p) => ({ ...p, [c.id]: v }));
                          validatePay(c.id, payAmounts[c.id] || '', v);
                        }}
                    ...
                    <div className="flex gap-2">
                      {c.minPayment &&
                        ...
                        (minSatisfied ? (
                          ...
                          <button
                            onClick={() => {
                              const minPayment = c.minPayment;
                              // Unreachable: this button only renders when c.minPayment > 0.
                              if (minPayment === undefined) return;
                              const src = paySources[c.id] || '';
                              if (!src) {
                                setPayErrors((p) => ({ ...p, [c.id]: 'Select source' }));
                                showToast('error', 'Select source');
                                return;
                              }
                              const source = fundingAccounts.find((a) => `${a.type}-${a.id}` === src);
                              if (!source || source.isFrozen || source.balance < minPayment) {
                                showToast('error', `Need ${currency}${minPayment} min`);
                                return;
                              }
                              const [t, ...rest] = src.split('-');
                              const id = rest.join('-');
                              onPayCard(c.id, minPayment, id, t as 'cash' | 'card');
                              setPayAmounts((p) => {
                                const cp = { ...p };
                                delete cp[c.id];
                                return cp;
                      ...
                      <button
                        onClick={() => {
                          const src = paySources[c.id] || '';
                          if (!validatePay(c.id, '', src, true)) {
                            showToast('error', payErrors[c.id] || 'Select valid source');
                            return;
                          }
                          const [t, ...rest] = src.split('-');
                          const id = rest.join('-');
                          onPayCard(c.id, Math.abs(c.currentBalance), id, t as 'cash' | 'card');
                          setPayAmounts((p) => {
                            const cp = { ...p };
                            delete cp[c.id];
                            return cp;
                      ...
                      <button
                        onClick={() => {
                          const src = paySources[c.id] || '';
                          const amt = payAmounts[c.id] || '';
                          if (!validatePay(c.id, amt, src, false)) {
                            showToast('error', payErrors[c.id] || 'Enter amount & source');
                            return;
                          }
                          const [t, ...rest] = src.split('-');
                          const id = rest.join('-');
                          onPayCard(c.id, parseFloat(amt), id, t as 'cash' | 'card');
                          setPayAmounts((p) => {
                            const cp = { ...p };
                            delete cp[c.id];
                            return cp;
                    ...
                    {payErrors[c.id] && (
                      <span className="mono text-[11px] text-[var(--danger)] block text-right">{payErrors[c.id]}</span>
                    )}
                  </div>
                  ...
                  {showHistory[`${c.id}-purchases`] && (
                    <div className="space-y-1 max-h-48 overflow-y-auto scrollbar-none">
                      {cardPurchases.length === 0 ? (
                        ...
                        cardPurchases.map((p) => {
                          const hasInstallment = creditCardInstallments.some(
                            (inst) => inst.purchaseId === p.id && inst.status === 'active',
                          );
                          return (
                            <div
                              key={p.id}
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                ...
                                {p.description && (
                                  <span className="text-[var(--ink-3)] text-[10px] block truncate">
                                    {p.description}
                                  </span>
                                )}
                              </div>
                  ...
                  {creditCardInstallments.filter((inst) => inst.cardId === c.id && inst.status === 'active').length >
                    0 && (
      ...
      <form onSubmit={handleAddPurchase} className="card p-5 md:p-6 space-y-4">
        <div className="flex items-center gap-2 pb-3 border-b border-[var(--line)]">
          <span className="w-7 h-7 rounded-full bg-[var(--ink)] text-[var(--accent-fg)] grid place-items-center">
            <Plus size={12} />
          </span>
          <span className="eyebrow">Record swipe / purchase</span>
        </div>
        <div className="rainbow-bar opacity-60" />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="sm:col-span-2 flex flex-col gap-1.5">
            <label className="eyebrow normal-case">Credit account</label>
            <select
              ref={purchaseCardRef}
              value={purCardId}
              onChange={(e) => {
            ...
            {purchaseErrors.cardId && (
              <span className="mono text-[11px] text-[var(--danger)]">{purchaseErrors.cardId}</span>
            )}
          </div>
          <div className="sm:col-span-2 flex flex-col gap-1.5">
            ...
            {purchaseErrors.amount && (
              <span className="mono text-[11px] text-[var(--danger)]">{purchaseErrors.amount}</span>
            )}
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="eyebrow normal-case">Merchant</label>
            <input
            ...
            {purchaseErrors.merchant && (
              <span className="mono text-[11px] text-[var(--danger)]">{purchaseErrors.merchant}</span>
            )}
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="eyebrow normal-case">Reference</label>
            <input
              placeholder="Business dinner"
              value={purDesc}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="eyebrow normal-case">Date</label>
            <input type="date" className="input" value={purDate} onChange={(e) => setPurDate(e.target.value)} />
          </div>
        </div>
