...
export default function SettingsModal({
  ...
  const refreshAppLock = async () => {
    ...
    setAppLockStatus(
      status
        ? {
            appLockEnabled: status.appLockEnabled,
            lockOnOpen: status.lockOnOpen,
            hasPin: status.hasPin,
            pinEnabled: status.pinEnabled,
            biometricCount: status.biometricCount,
          }
        : null,
    );
    ...
    setDevices(
      devs.map((d) => ({
        id: d.id,
        label: d.userAgent.split(/[ (/]/)[0] || 'Device',
        lastUsed: new Date(d.lastUsedAt || d.createdAt).toLocaleDateString(),
      })),
    );
  ...
  const handlePushSync = async () => {
    ...
    if (!supabaseUrl.trim() || !supabaseKey.trim()) {
      setSyncStatus('error');
      setSyncMessage('Supabase URL and Anon Key are required.');
      return;
    }
    setSyncStatus('loading');
    setSyncMessage('Pushing ledger to cloud...');
    const res = await syncStateToSupabase(userEmail, state, true);
    if (res.success) {
      setSyncStatus('success');
      setSyncMessage('Backup published to cloud.');
    } else {
      setSyncStatus('error');
      setSyncMessage(res.error || 'Cloud write failed.');
    }
  };
  ...
  const handlePullSync = async () => {
    ...
    setSyncStatus('loading');
    setSyncMessage('Pulling ledger from cloud...');
    const res = await syncStateFromSupabase(userEmail);
    if (res.success && res.state) {
      updateState(() => res.state!);
      setSyncStatus('success');
      setSyncMessage('Cloud state restored.');
    } else {
      setSyncStatus('error');
      setSyncMessage(res.error || 'No backup found.');
    }
  };
  ...
  const handleWipeDatabase = async () => {
    if (!userEmail) {
      setSyncStatus('error');
      setSyncMessage('Email required.');
      return;
    }
    setSyncStatus('loading');
    setSyncMessage('Sending verification code...');
    setPurgeLoading(true);
    setPurgeError(null);
    try {
      const token = authSession.getToken() || '';
      const res = await fetch(apiUrl('/api/auth/send-delete-otp'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        setPurgeDevOtp(data.devOtp || null);
        setSyncStatus('success');
        setSyncMessage(data.emailSent ? 'Code sent to your email.' : 'Dev code generated.');
        showToast('Passcode sent. Enter the code below.', 'success');
      } else {
        setSyncStatus('error');
        setSyncMessage(data.error || 'Failed to send code.');
        showToast(data.error || 'Failed.', 'error');
      }
    } catch (err: unknown) {
      setPurgeLoading(false);
      setSyncStatus('error');
      setSyncMessage(err instanceof Error ? err.message : 'Network error.');
  ...
  const handleConfirmPurge2FA = async () => {
    if (!purgeOtp.trim()) {
      setPurgeError('Enter the 6-digit code.');
      return;
    }
    setPurgeLoading(true);
    setPurgeError(null);
    setSyncStatus('loading');
    setSyncMessage('Verifying code...');
    try {
      const token = authSession.getToken() || '';
      const res = await fetch(apiUrl('/api/auth/verify-delete-otp'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      ...
      if (!res.ok || !data.success) {
        setPurgeLoading(false);
        setSyncStatus('error');
        const m = data.error || 'Invalid or expired code.';
        setSyncMessage(m);
        setPurgeError(m);
        showToast(m, 'error');
        return;
      ...
      if (!result.success) {
        setSyncStatus('error');
        setSyncMessage(result.error || 'Cloud truncate failed.');
        showToast('Cloud sync failure.', 'error');
        return;
      }
      updateState((prev) => ({
        ...prev,
      ...
      setSyncStatus('success');
      setSyncMessage('Ledger purged.');
      showToast('Ledger purged.', 'success');
    } catch (err: unknown) {
      setPurgeLoading(false);
      setSyncStatus('error');
  ...
  const handleSetPin = async () => {
    ...
    if (/^(0+|\d)\1*$/.test(pin) || pin === '1234' || pin === '0000' || pin === '4321') {
    ...
    if (r.ok) {
      setAppLockMsg({ kind: 'success', text: 'PIN set. App lock is now active.' });
      setNewPin('');
  ...
  const handleDisablePin = async () => {
    ...
    setAppLockBusy(false);
    if (r.ok) {
      setAppLockMsg({ kind: 'success', text: 'PIN disabled.' });
      await refreshAppLock();
  ...
  const handleDisableAll = async () => {
    ...
    setAppLockBusy(false);
    if (a.ok && b.ok) {
      setAppLockMsg({ kind: 'success', text: 'App lock disabled and all trusted devices removed.' });
      setNewPin('');
      await refreshAppLock();
  ...
  const handleSetLockOnOpen = async (enabled: boolean) => {
    ...
    if (r.ok) {
      setAppLockMsg({
        kind: 'success',
        text: enabled ? 'PIN will now be asked every time the app opens.' : 'PIN only asked on new/unknown devices.',
      });
      await refreshAppLock();
  ...
  const handleRegisterBiometric = async () => {
    ...
    setAppLockBusy(false);
    if (r.ok) {
      setAppLockMsg({ kind: 'success', text: 'Biometric added. You can now unlock with this device.' });
      setDeviceLabel('');
      await refreshAppLock();
  ...
  const handleRemoveBiometric = async (credentialId: string) => {
    ...
    for (const id of ids) {
      ...
      if (!r.ok) {
        ok = false;
        err = r.error || 'Failed to remove biometric.';
      }
    }
    setAppLockBusy(false);
    setConfirmRemoveBiometric(null);
    if (ok) {
      setAppLockMsg({ kind: 'success', text: 'Biometric removed.' });
      await refreshAppLock();
  ...
  const handleRevokeDevice = async (id: string) => {
    ...
    setAppLockBusy(false);
    if (r.ok) {
      setAppLockMsg({ kind: 'success', text: 'Device removed.' });
      await refreshAppLock();
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            onClick={onClose}
            className="fixed inset-0 z-40 bg-[var(--ink)]/40 backdrop-blur-[2px]"
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 w-full max-w-[600px] bg-[var(--surface)] border-l border-[var(--line)] z-50 flex flex-col shadow-2xl"
          ...
          >
            <div className="px-6 h-14 flex items-center justify-between border-b border-[var(--line)] bg-[var(--surface)]/80 backdrop-blur shrink-0">
              <div className="flex items-center gap-2.5">
                <span className="w-7 h-7 rounded-full bg-[var(--surface-2)] border border-[var(--line)] flex items-center justify-center text-[var(--ink-2)]">
                  <Settings size={13} />
                </span>
                <div>
              ...
              <button
                ...
                className="w-10 h-10 rounded-full bg-[var(--surface-2)] border border-[var(--line)] text-[var(--ink-2)] hover:text-[var(--ink)] flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ink)]"
              >
                <X size={14} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
              ...
              <section className="space-y-3">
                <div className="eyebrow flex items-center justify-between">
                  ...
                  <span className="mono text-[10px] font-normal normal-case tracking-normal px-2 py-0.5 rounded-full border border-[var(--line)] bg-[var(--surface-2)] text-[var(--ink-2)]">
                    Flutter ready
                  </span>
                </div>
                <div className="card-flat p-5 space-y-4">
                  <p className="text-[12px] leading-5 text-[var(--ink-2)]">
                    Connect Supabase to sync your ledger. Credentials are locked to build-time env vars for safety.
                  </p>
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        className="input opacity-60 cursor-not-allowed mono text-[12px]"
                      />
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="input opacity-60 cursor-not-allowed mono text-[12px] pr-9"
                        />
                        <button
                          type="button"
                          onClick={() => setShowKey((v) => !v)}
                          aria-label={showKey ? 'Hide key' : 'Show key'}
                          className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full text-[var(--ink-3)] hover:text-[var(--ink)] hover:bg-[var(--surface-2)]"
                        >
                    ...
                    <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-2)] px-3 py-2.5 flex gap-2 text-[11px] leading-4 text-[var(--ink-2)]">
                      <Lock size={12} className="shrink-0 mt-0.5 text-[var(--ink-3)]" />
                      <span>Connection credentials are locked to environment variables.</span>
                    </div>
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          saveSupabaseConfig(cfg.url, cfg.key, v);
                        }}
                        className="w-5 h-5 rounded border-[var(--line)] bg-[var(--surface)] accent-[var(--ink)]"
                      />
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <button
                      ...
                      className="btn-ghost justify-center inline-flex items-center gap-1.5 text-[12px] disabled:opacity-50"
                    >
                      {syncStatus === 'loading' ? (
                  ...
                  {syncMessage && (
                    <div
                      className={`rounded-xl border px-3 py-2.5 flex gap-2 text-[12px] leading-5 ${syncStatus === 'success' ? 'bg-[var(--success-bg)] border-[var(--line)] text-[var(--success)]' : syncStatus === 'error' ? 'bg-[var(--danger-bg)] border-[var(--line)] text-[var(--danger)]' : 'bg-[var(--surface-2)] border-[var(--line)] text-[var(--ink-2)]'}`}
                    >
              ...
              <section className="space-y-3">
                <div className="eyebrow flex items-center justify-between">
                  <span className="inline-flex items-center gap-1.5">
                    <Fingerprint size={11} /> App Lock
                  </span>
                  <span className="mono text-[10px] font-normal normal-case tracking-normal px-2 py-0.5 rounded-full border border-[var(--line)] bg-[var(--surface-2)] text-[var(--ink-2)]">
                    {appLockStatus?.appLockEnabled ? 'Active' : 'Off'}
                  </span>
                </div>
                <div className="card-flat p-5 space-y-4">
                  ...
                  {appLockStatus && (
                    <>
                      <div className="flex items-start gap-2.5 text-[12px] leading-5 text-[var(--ink-2)]">
                        ...
                        <p>
                          App lock adds a PIN or biometric layer on top of your login, so a stolen/borrowed session
                          still can't be read until you unlock.
                        </p>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-2)] px-2 py-2.5">
                          <p className="text-[15px] font-bold text-[var(--ink)]">
                            {appLockStatus.pinEnabled ? 'On' : 'Off'}
                          </p>
                      ...
                      <div className="space-y-2">
                        <p className="text-[12px] font-semibold text-[var(--ink)] inline-flex items-center gap-1.5">
                        ...
                        {appLockStatus.hasPin ? (
                          ...
                          <div className="space-y-2">
                            <div>
                              <label htmlFor="settings-pin" className="eyebrow block mb-1.5">
                                Set a 4–6 digit PIN
                              </label>
                              <div className="flex gap-2">
                                <input
                                  ...
                                  placeholder="••••"
                                  className="input mono text-center tracking-[0.35em] flex-1"
                                />
                                <button
                                  type="button"
                                  onClick={handleSetPin}
                                  disabled={appLockBusy || newPin.length < 4}
                      ...
                      <div className="flex items-center justify-between gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-2)] px-3 py-2.5">
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] leading-4 text-[var(--ink-3)] mt-0.5">
                              Require the PIN every time the app opens, even on a remembered device.
                            </p>
                          </div>
                        </div>
                        <button
                        ...
                        >
                          <span
                            className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${appLockStatus.lockOnOpen ? 'translate-x-5' : ''}`}
                          />
                        </button>
                      </div>
                        ...
                        {appLockStatus.biometricCount > 0 ? (
                          confirmRemoveBiometric ? (
                            <div className="rounded-xl border border-[var(--line)] bg-[var(--danger-bg)] p-3 space-y-2">
                              <p className="text-[11px] leading-4 text-[var(--ink-2)]">
                                Remove all biometric unlock methods for this account?
                              </p>
                              <div className="grid grid-cols-2 gap-2">
                                <button
                                  type="button"
                                  onClick={() => setConfirmRemoveBiometric(null)}
                                  className="btn-ghost justify-center text-[12px]"
                                  disabled={appLockBusy}
                                >
                                  Cancel
                                </button>
                                <button
                                  ...
                                  onClick={() => {
                                    const id =
                                      confirmRemoveBiometric === '__all__'
                                        ? biometricCredIds[0] || ''
                                        : confirmRemoveBiometric;
                                    if (id) handleRemoveBiometric(id);
                                  }}
                                  className="btn-primary justify-center text-[12px] bg-[var(--danger)] border-[var(--danger)] hover:brightness-95 disabled:opacity-50"
                                  disabled={appLockBusy}
                                >
                                  Remove
                                </button>
                            ...
                            <button
                              type="button"
                              onClick={() => setConfirmRemoveBiometric('__all__')}
                              disabled={appLockBusy}
                              className="btn-ghost w-full justify-center text-[12px] text-[var(--danger)] disabled:opacity-50"
                            >
                              Remove biometric unlock ({appLockStatus.biometricCount})
                            </button>
                          )
                        ) : (
                          <div className="space-y-2">
                            <p className="text-[11px] leading-4 text-[var(--ink-3)]">
                              {biometricSupported
                                ? 'Add fingerprint / face unlock using this device.'
                                : 'This device does not support platform biometrics.'}
                            </p>
                            <div className="flex gap-2">
                              <input
                                value={deviceLabel}
                                onChange={(e) => setDeviceLabel(e.target.value)}
                                placeholder="Device label (this phone)"
                                maxLength={60}
                              ...
                              <button
                                ...
                                disabled={appLockBusy || !biometricSupported}
                                className="btn-primary disabled:opacity-50"
                              >
                                Add
                              </button>
                      ...
                      <div className="space-y-2">
                        <p className="text-[12px] font-semibold text-[var(--ink)] inline-flex items-center gap-1.5">
                          <Smartphone size={12} /> Trusted devices
                        </p>
                        {devices.length === 0 ? (
                          <p className="text-[11px] leading-4 text-[var(--ink-3)]">
                            No trusted devices. On login, tick "Remember this device" to skip the lock on this browser.
                          </p>
                        ) : (
                          <div className="space-y-2">
                            {devices.map((d) => (
                              <div
                                key={d.id}
                                className="flex items-center justify-between gap-2 rounded-xl border border-[var(--line)] bg-[var(--surface-2)] px-3 py-2 text-[12px]"
                              >
                                <span className="truncate flex items-center gap-1.5">
                                  <Smartphone size={11} className="shrink-0 text-[var(--ink-3)]" />
                                  {d.label}{' '}
                                  <span className="mono text-[10px] text-[var(--ink-3)] shrink-0">{d.lastUsed}</span>
                                </span>
                                <button
                                  type="button"
                                  onClick={() => handleRevokeDevice(d.id)}
                                  disabled={appLockBusy}
                                  className="text-[11px] text-[var(--danger)] hover:underline shrink-0 disabled:opacity-40"
                                >
                                  Remove
                                </button>
                              </div>
                            ...
                            <button
                              type="button"
                              onClick={async () => {
                                const r = await revokeAllDevices(userEmail);
                                if (r.ok) {
                                  setAppLockMsg({ kind: 'success', text: 'All trusted devices removed.' });
                                  await refreshAppLock();
                                }
                              }}
                              disabled={appLockBusy}
                      ...
                      <div className="pt-3 border-t border-[var(--line)]">
                        <button
                          ...
                          disabled={appLockBusy}
                          className="w-full rounded-full border border-[var(--line)] bg-[var(--danger-bg)] text-[var(--danger)] hover:brightness-95 px-4 py-2.5 text-[12px] font-semibold inline-flex items-center justify-center gap-2 disabled:opacity-50"
                        >
                          <Shield size={12} /> Disable app lock everywhere
                        </button>
                      </div>
                    </>
                  )}
                  {appLockMsg && (
                    <div
                      className={`rounded-xl border px-3 py-2.5 flex gap-2 text-[12px] leading-5 ${appLockMsg.kind === 'success' ? 'bg-[var(--success-bg)] border-[var(--line)] text-[var(--success)]' : appLockMsg.kind === 'error' ? 'bg-[var(--danger-bg)] border-[var(--line)] text-[var(--danger)]' : 'bg-[var(--surface-2)] border-[var(--line)] text-[var(--ink-2)]'}`}
                    >
                      {appLockMsg.kind === 'success' ? (
                        <Check size={13} className="shrink-0 mt-0.5" />
                      ) : appLockMsg.kind === 'error' ? (
              ...
              <section className="space-y-2">
                ...
                <div className="card-flat overflow-hidden">
                  <button
                    ...
                    className="w-full px-4 h-11 flex items-center justify-between text-[12px] font-semibold text-[var(--ink)] hover:bg-[var(--surface-2)] transition-colors"
                  >
                    <span className="inline-flex items-center gap-2 text-[var(--ink-2)]">
                      <Zap size={12} />
                      <span className="text-[var(--ink)]">Sync with Flutter (Dart)</span>
                    </span>
                  ...
                  <AnimatePresence>
                    {expandedSection === 'flutter' && (
                      <motion.div
                        ...
                        transition={{ duration: 0.18 }}
                        className="border-t border-[var(--line)] bg-[var(--surface-2)] p-4"
                      >
                        <p className="text-[11px] leading-4 text-[var(--ink-2)] mb-2">
                          Add supabase_flutter and use this helper.
                        </p>
                        <div className="relative">
                          <pre className="mono text-[11px] leading-4 bg-[var(--surface)] border border-[var(--line)] rounded-xl p-3 overflow-x-auto max-h-[260px] text-[var(--ink-2)] whitespace-pre">
                            {flutterCode}
                          </pre>
                          <button
                            ...
                            aria-label="Copy"
                          >
                            {flutterCopied ? <Check size={11} /> : <Copy size={11} />}
                          </button>
                        </div>
                      </motion.div>
                    )}
              ...
              <section className="space-y-3">
                <p className="eyebrow inline-flex items-center gap-1.5">
                  <Shield size={11} /> Identity &amp; preferences
                </p>
                <div className="card-flat p-5 space-y-4">
                  <div>
                    <p className="text-[12px] font-semibold text-[var(--ink)]">Signed in as</p>
                    <p className="mono text-[12px] text-[var(--ink-2)] break-all mt-1 inline-flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <button type="button" onClick={toggleTheme} className="btn-ghost w-full justify-between">
                      ...
                      <span className="mono text-[10px] px-2 py-0.5 rounded-full border border-[var(--line)] bg-[var(--surface-2)]">
                        Toggle
                      </span>
                    </button>
                  </div>
                ...
                <div className="card-flat p-5 space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="block w-full text-[12px] text-[var(--ink-2)] file:mr-3 file:btn-ghost file:py-1.5 file:px-3 file:text-[12px]"
                    />
                  </div>
                  {/* CSV exports – one-click spreadsheet downloads per collection */}
                  <div className="pt-3 border-t border-[var(--line)] space-y-2">
                    <p className="eyebrow inline-flex items-center gap-1.5">
                      <FileDown size={11} /> CSV exports (spreadsheet)
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => exportCashAccountsCSV(state.cashAccounts, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Cash accounts
                      </button>
                      <button
                        onClick={() => exportCardsCSV(state.cards, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Cards
                      </button>
                      <button
                        onClick={() => exportDebtsCSV(state.debts, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Debts
                      </button>
                      <button
                        onClick={() => exportLoansCSV(state.loansGiven, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Loans
                      </button>
                      <button
                        onClick={() => exportSubscriptionsCSV(state.subscriptions, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Subscriptions
                      </button>
                      <button
                        onClick={() => exportBudgetsCSV(state.budgets || [], state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Budgets
                      </button>
                      <button
                        onClick={() => exportGoalsCSV(state.savingsGoals || [], state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Goals
                      </button>
                      <button
                        onClick={() => exportIncomesCSV(state.incomes, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Incomes
                      </button>
                      <button
                        onClick={() => exportExpensesCSV(state.expenses, state.currency)}
                        className="btn-ghost justify-center text-[11px] px-2 py-2"
                      >
                        Expenses
                      </button>
                  ...
                  {!showPurge2FA ? (
                    <div className="pt-3 border-t border-[var(--line)] space-y-2">
                      <button
                        ...
                        className="w-full rounded-full border border-[var(--line)] bg-[var(--danger-bg)] text-[var(--danger)] hover:brightness-95 px-4 py-2.5 text-[12px] font-semibold inline-flex items-center justify-center gap-2"
                      >
                        <Shield size={12} /> Wipe cloud &amp; local state
                      </button>
                      <p className="mono text-[10px] text-[var(--ink-3)] text-center">
                        Irreversible. Requires email 2FA.
                      </p>
                    </div>
                  ) : (
                    <div className="rounded-xl border border-[var(--line)] bg-[var(--danger-bg)] p-3.5 space-y-3">
                      <p className="text-[12px] font-semibold text-[var(--danger)] inline-flex items-center gap-1.5">
                        <AlertCircle size={13} /> Verify to wipe
                      </p>
                      <p className="text-[11px] leading-4 text-[var(--ink-2)]">
                        Code sent to <span className="mono text-[var(--ink)]">{userEmail}</span>.
                      </p>
                      <input
                        type="text"
                        maxLength={6}
                        ...
                        className="input mono text-center tracking-[0.35em]"
                        disabled={purgeLoading}
                      />
                      {purgeError && <p className="text-[11px] text-[var(--danger)]">{purgeError}</p>}
                      {purgeDevOtp && (
                        <p className="mono text-[11px] text-[var(--ink-2)]">
                          Dev OTP: <span className="text-[var(--ink)] font-semibold">{purgeDevOtp}</span>
                        </p>
                      )}
                      <div className="grid grid-cols-2 gap-2">
                        <button
                        ...
                        <button
                          ...
                          className="btn-primary justify-center bg-[var(--danger)] border-[var(--danger)] hover:brightness-95 disabled:opacity-50"
                          disabled={purgeLoading}
                        >
                          {purgeLoading ? 'Purging...' : 'Confirm wipe'}
                        </button>
                      </div>
                    </div>
                  )}
              ...
              <div className="card-flat p-4 flex gap-2.5 text-[11px] leading-5 text-[var(--ink-2)]">
                ...
                <ul className="space-y-1 list-disc list-inside">
                  <li>Incomes credit the chosen account.</li>
                  <li>Expenses check balance before posting.</li>
                  <li>Debt payments update remaining in real time.</li>
                </ul>
              </div>
              ...
              <button
                ...
                onClick={onLogout}
                className="btn-ghost w-full justify-center inline-flex items-center gap-2 text-[var(--danger)] border-[var(--line)] hover:bg-[var(--danger-bg)]"
              >
                <LogOut size={13} /> Log out
              </button>
            </div>

            <div className="px-6 h-10 border-t border-[var(--line)] flex items-center justify-center mono text-[10px] text-[var(--ink-3)] shrink-0">
              Secured · local-first
            </div>
          </motion.div>
        </>
