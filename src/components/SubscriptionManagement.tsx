...
export default function SubscriptionManagement({
  ...
  const validateForm = (name: string, amtStr: string, sub: boolean) => {
    ...
    if (sub || amtStr) {
      if (!amtStr) errs.amount = 'Billing sum required';
        ...
        if (isNaN(num)) errs.amount = 'Must be number';
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    if (!isValid) {
      if (!subName.trim()) nameInputRef.current?.focus();
    ...
    setSubmitted(false);
    setErrors({});
    showToast('success', 'Subscription registered.');
  };
  ...
  const executePayment = () => {
    ...
    if (availableBalance < sub.amount + chargeVal) {
      showToast(
        'error',
        `Insufficient ${currency}${(sub.amount + chargeVal).toLocaleString()}, have ${currency}${availableBalance.toLocaleString()}`,
      );
      return;
    }
    ...
    <div className="space-y-6" id="subscription-management-section">
      <div className="gradient-card p-6 overflow-hidden" style={{ background: 'var(--gradient-card-dark)' }}>
        <div className="flex flex-col sm:flex-row justify-between gap-4 relative z-10">
          <div>
            ...
            <h2 className="text-[22px] font-bold tracking-tight mt-1 text-white">Subscriptions</h2>
            <p className="text-[13px] mt-1 text-white/60">Ledger of recurring charges — status pills, due mono.</p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5 relative z-10">
          <div className="rounded-[14px] p-3 bg-white/10 border border-white/10">
            <p className="eyebrow !text-white/60 !text-[9px]">Active</p>
            <p className="mono text-[18px] font-bold mt-1 text-white">
              {subscriptions.filter((s) => s.status === 'Active').length}
            </p>
          </div>
          <div className="rounded-[14px] p-3 bg-white/10 border border-white/10">
            <p className="eyebrow !text-white/60 !text-[9px]">Monthly total</p>
          ...
          <div className="rounded-[14px] p-3 bg-white/10 border border-white/10">
            <p className="eyebrow !text-white/60 !text-[9px]">Upcoming dues</p>
            <p className="mono text-[15px] font-bold mt-1 text-white">
              {
      ...
      {isAdding && (
        <form onSubmit={handleCreate} className="card p-6 space-y-4">
          ...
          <div>
            <label className="eyebrow block mb-2">Service name</label>
            <input
              ref={nameInputRef}
              type="text"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="eyebrow block mb-2">Billing cycle</label>
              <select
                value={billingCycle}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="eyebrow block mb-2">Next due</label>
          ...
          <div>
            <label className="eyebrow block mb-2">Instance type (optional)</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <select
                value={instanceTypeSelection}
                onChange={(e) => {
                  const v = e.target.value;
                  setInstanceTypeSelection(v);
                  if (v !== 'custom') setInstanceType(v === 'none' ? '' : v);
                  else setInstanceType('');
                }}
                className="input"
              >
                <option value="none">Not a server</option>
              ...
              {instanceTypeSelection === 'custom' ? (
                ...
                <span className="mono text-[11px] self-center" style={{ color: 'var(--ink-3)' }}>
      ...
      {selectedSubscription && (
        <div className="card p-6 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setSelectedSubId(null)} className="btn-ghost !py-1.5 !px-3 text-[11px]">
              Cancel
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <optgroup label="Cards">
                  {cards
                    ...
                    .map((c) => (
                      <option key={c.id} value={`${c.id}:card`}>
                        {c.bankName} - {c.cardName} ({currency}
            ...
            <div>
              <label className="eyebrow block mb-2">Payment date</label>
      ...
      {subscriptions.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4" id="subscriptions-grid">
          {sortedSubscriptions.map((sub) => {
            ...
            const pillStyle =
              sub.status === 'Active'
                ? { borderColor: 'var(--ink)', color: 'var(--ink)', background: 'var(--surface-2)' }
            ...
            return (
              <motion.div
                ...
                className={`card p-5 space-y-4 overflow-hidden relative ${selectedSubId === sub.id ? '!border-[var(--ink)]' : ''}`}
              >
                ...
                <div className="flex justify-between items-start gap-3">
                  <div className="flex gap-3 items-center">
                    ...
                    <div>
                      ...
                      <p
                        className="mono text-[11px] mt-0.5 flex flex-wrap gap-1.5 items-center"
                        style={{ color: 'var(--ink-3)' }}
                      >
                        <span className="px-2 py-0.5 rounded-full" style={{ border: '1px solid var(--line)' }}>
                          {sub.category}
                        </span>
                        {sub.billingCycle}
                        {sub.instanceType && (
                  ...
                  <span className="mono text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0" style={pillStyle}>
                    {status.label}
                  </span>
                </div>
                <div className="ledger-rule" />
                <div className="flex justify-between items-end">
                  <span className="eyebrow">Next renewal</span>
                  <span className="mono text-[12px] font-bold">{sub.dueDate}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2" style={{ borderTop: '1px solid var(--line)', paddingTop: 12 }}>
                  {sub.status === 'Active' ? (
                    <button
                      ...
                      className="btn-ghost !py-2.5 text-[12px] inline-flex items-center justify-center gap-1"
                    >
                      <Pause size={12} />
                      Pause
                    </button>
                    ...
                    <button
                      ...
                      className={
                        selectedSubId === sub.id
                          ? 'btn-primary !py-2.5 text-[12px] inline-flex items-center justify-center gap-1'
                          : 'btn-ghost !py-2.5 text-[12px] inline-flex items-center justify-center gap-1'
                      }
                    >
                      <DollarSign size={12} />
                      Settle
                  ...
                  <button
                    ...
                    className="btn-ghost col-span-2 !py-2.5 text-[12px] inline-flex items-center justify-center gap-1"
