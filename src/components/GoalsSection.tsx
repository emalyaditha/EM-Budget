        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mono text-[11px] mt-2" style={{ color: 'var(--ink-2)' }}>
              {activeGoalsCount} goals · {completedGoalsCount} completed · {currency}
              {totalSavedValue.toLocaleString()} saved
            </p>
          </div>
            ...
            >
              <div className="bg-[var(--surface-2)] border border-[var(--line)] rounded-[16px] p-4 flex flex-col gap-3">
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button onClick={() => setIsAddModalOpen(false)} className="btn-ghost !py-2 !px-4 text-[12px]">
                    Cancel
                  </button>
                  <button onClick={handleCreateGoal} className="btn-primary !py-2 !px-5 text-[12px]">
                    Create goal
                  </button>
                </div>
              </div>
                      ...
                      >
                        <div className="pt-3 mt-1 flex flex-col gap-3" style={{ borderTop: '1px solid var(--line)' }}>
                          ...
                          <div className="flex flex-wrap gap-2 items-center">
                            ...
                            <span className="mono text-[10px]" style={{ color: 'var(--ink-3)' }}>
                              {currency}
                              {goal.current.toLocaleString()} / {currency}
                              {goal.target.toLocaleString()}
                            </span>
                            {onRemoveGoal && (
                              <button
                                onClick={() => {
                                  if (deleteConfirmId === goal.id) {
                                    ...
                                    setTimeout(() => setDeleteConfirmId((c) => (c === goal.id ? null : c)), 3500);
                                  }
                                }}
                                className="ml-auto inline-flex items-center gap-1 mono text-[11px] px-2.5 py-1 rounded-full"
                                style={{
                                  border: '1px solid var(--line)',
                                  background: deleteConfirmId === goal.id ? 'var(--danger-bg)' : 'transparent',
                                  color: deleteConfirmId === goal.id ? 'var(--danger)' : 'var(--ink-2)',
                                }}
                              >
                                <Trash2 size={11} />
                                {deleteConfirmId === goal.id ? 'Confirm' : 'Delete'}
                              </button>
                            )}
                          </div>
                        </div>
          ...
          {justCommittedGoal && (
            ...
            >
              <div className="bg-[var(--surface-2)] border border-[var(--line)] rounded-[16px] p-3 flex items-center justify-between gap-3">
                ...
                <span
                  className="mono text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0"
        ...
        {isFundModalOpen && (
          ...
          >
            ...
            >
              ...
              <h4 className="text-[15px] font-bold mt-1">
                {fundAction === 'add' ? 'Assign to goal' : 'Withdraw from goal'}
              </h4>
              <div className="mt-5 space-y-4">
                ...
                <div>
                  <label className="eyebrow block mb-2">
                    {fundAction === 'add' ? 'Source wallet' : 'Return to wallet'}
                  </label>
                  <select
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button onClick={() => setIsFundModalOpen(false)} className="btn-ghost">
                    Cancel
                  </button>
                  <button onClick={handleFundGoalSubmit} className="btn-primary">
                    Confirm
                  </button>
                </div>
              </div>
