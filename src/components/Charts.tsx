  ...
  if (!data || data.length === 0) {
    return (
      <div className="card p-8 text-center min-h-[160px] grid place-items-center overflow-hidden relative">
        ...
        <div>
          <p className="text-[13px] font-semibold">No trend data</p>
          <p className="mono text-[11px] mt-1" style={{ color: 'var(--ink-3)' }}>
            Add transactions to map trend.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="card p-5 overflow-hidden relative">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full overflow-visible">
        <defs>
          <linearGradient id="trendMono" x1="0" y1="0" x2="0" y2="1">
        ...
        {data.map((d, idx) => {
          ...
          return (
            <g key={idx}>
              <circle cx={x} cy={y} r="3.5" fill="var(--surface)" stroke="var(--ink)" strokeWidth="1.5" />
  ...
  return (
    <div className="card p-5 space-y-4 overflow-hidden relative">
      ...
      <div className="flex justify-between items-center">
        <p className="eyebrow">Inflow vs outflow</p>
        <span className="pill !py-1 !px-2 mono !text-[10px]">Statement</span>
      </div>
      <div>
        <div className="flex justify-between mono text-[11px] mb-1.5">
          <span style={{ color: 'var(--ink-2)' }}>Incomes</span>
          <span className="font-bold">
            {currency} {income.toLocaleString()} ({incomePct}%)
          </span>
        </div>
        <div className="h-2 w-full rounded-full bg-[var(--surface-3)] overflow-hidden">
          <div className="h-full rounded-full bar-mint" style={{ width: `${incomePct}%` }} />
        </div>
      </div>
      <div>
        <div className="flex justify-between mono text-[11px] mb-1.5">
          <span style={{ color: 'var(--ink-2)' }}>Expenses</span>
          <span className="font-bold">
            {currency} {expense.toLocaleString()} ({expensePct}%)
          </span>
        </div>
        <div className="h-2 w-full rounded-full bg-[var(--surface-3)] overflow-hidden">
          <div className="h-full rounded-full bar-pink" style={{ width: `${expensePct}%` }} />
        </div>
      </div>
    </div>
  );
...
function pastelClass(idx: number) {
  ...
  if (categories.length === 0) {
    return (
      <div className="card p-6 text-center min-h-[180px] grid place-items-center">
        <div>
          <p className="eyebrow">Category spread</p>
        ...
        <div className="w-full mt-4 space-y-1.5 max-h-[160px] overflow-y-auto pr-1">
          {categories.map((cat, idx) => {
            ...
            return (
              <div
                key={cat.name}
                className="grid grid-cols-[8px_1fr_auto_auto] items-center gap-2 h-8 px-2.5 rounded-xl"
              ...
              >
                ...
                <span className="mono text-[11px] font-bold" style={{ color: 'var(--ink-2)' }}>
                  {percentage}%
                </span>
