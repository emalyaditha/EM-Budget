...
export function TrendAnalysisChart({ data, currency }: { data: { date: string; value: number }[]; currency: string }) {
  if (!data || data.length === 0) {
    return (
      <div className="w-full bg-card-50 border border-default rounded-[28px] p-8 shadow-xl text-center flex flex-col items-center justify-center min-h-[160px]">
        <span className="text-muted text-sm font-medium">No trend data available</span>
        <span className="text-muted text-xs mt-1">Add transaction logs to map assets over time.</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full bg-card-50 border border-default rounded-[28px] p-6 shadow-xl">
      <div className="flex justify-between items-center mb-4">
        <span className="text-xs font-bold text-primary font-sans">Net Asset Trend (6 Days)</span>
        <span className="text-[10px] text-primary bg-blue-950/40 px-2.5 py-1 rounded-md border border-blue-900/40 font-mono font-bold flex items-center gap-1">
      ...
      <div className="flex justify-between text-[10px] text-muted font-mono mt-3 px-1.5">
        {data.map((d, idx) => {
          let label = d.date;
          try {
          ...
          return (
            <span key={idx} className="text-secondary font-semibold text-center" title={d.date}>
              {label}
            </span>
          );
        })}
...
export function IncomeVsExpenseBar({ income, expense, currency }: { income: number; expense: number; currency: string }) {
  ...
  return (
    <div className="bg-card-50 border border-default rounded-[28px] p-6 shadow-xl">
      <h3 className="text-sm font-bold text-primary mb-4 flex justify-between items-center font-sans leading-snug">
        <span>Inflow vs Outflow Contrast</span>
        <span className="text-[9px] uppercase font-mono text-secondary tracking-wider">Statement breakdown</span>
      </h3>
      <div className="space-y-4.5">
        {/* Income Bar */}
        <div>
          <div className="flex justify-between text-xs mb-1.5">
            <span className="text-secondary font-medium">Monthly Incomes</span>
        ...
        <div>
          <div className="flex justify-between text-xs mb-1.5">
            <span className="text-secondary font-medium">Invoices & Bills Expenses</span>
            <span className="text-rose-500 dark:text-danger font-mono font-bold">{currency} {expense.toLocaleString()} ({expensePct}%)</span>
          </div>
          <div className="w-full h-8 bg-primary border border-subtle dark:border-default rounded-xl overflow-hidden flex items-center relative chart-progress-track">
            <div
            ...
            />
            <span className="absolute left-3 text-[10px] font-mono text-secondary font-bold mix-blend-difference text-primary-forced">CHARGES</span>
          </div>
        </div>
      </div>
    </div>
...
export function CategorySpreadAnalysis({ 
  ...
  if (categories.length === 0) {
    return (
      <div 
        className="border border-default rounded-[28px] p-5 text-center flex flex-col items-center justify-center min-h-[180px] bg-card shadow-xl"
        style={{
          backdropFilter: 'blur(20px)',
        }}
      >
        <span className="text-secondary dark:text-primary/45 text-[11px] font-bold tracking-[2px] uppercase">Category Spread Analysis</span>
        <span className="text-muted dark:text-primary/60 text-xs mt-2 max-w-[200px]">No expense data logged to build category distributions</span>
      </div>
    );
  }
  ...
  return (
    <div 
      className="border border-default rounded-[20px] p-4.5 w-full max-w-[380px] mx-auto sm:mx-0 relative overflow-hidden transition-all duration-300 text-left select-none flex flex-col justify-between bg-card shadow-xl"
      style={{
        backdropFilter: 'blur(20px)',
      }}
    >
      ...
      <div className="mb-3">
        <h3 className="text-[14px] sm:text-[15px] font-bold text-primary dark:text-primary tracking-tight leading-tight select-all">
          Category Spread Analysis
        </h3>
        <p className="text-[11px] font-normal text-muted dark:text-primary/50 select-all">
        ...
        <div className="flex justify-center items-center shrink-0 mb-4 w-full">
          <div className="relative w-[130px] h-[130px] sm:w-[140px] sm:h-[140px] transition-all duration-300">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none px-2 text-center">
                <span className="text-[9px] text-muted dark:text-primary/50 font-bold uppercase tracking-[1px] leading-none mb-0.5">
                  TOTAL
                </span>
                <span className="text-[15px] sm:text-[17px] font-bold text-primary dark:text-primary font-mono leading-none tracking-tight select-all truncate max-w-full">
        ...
        <div className="w-full flex flex-col gap-1.5 max-h-[160px] overflow-y-auto pr-1.5 select-all scrollbar-thin">
          {categories.map((cat, idx) => {
            ...
            return (
              <div 
                key={cat.name} 
                className="grid grid-cols-[8px_minmax(0,1fr)_auto_auto] items-center gap-2 h-9 py-1 px-2.5 bg-surface0/5 dark:bg-white/[0.01] hover:scale-[1.01] hover:bg-surface0/10 dark:hover:bg-white/[0.03] transition-all duration-200 cursor-pointer rounded-lg animate-brand-fade-up select-all border border-subtle/50 dark:border-white/[0.01]"
                style={{
              ...
              >
                ...
                {/* Category Name */}
                <span 
                  className="text-[11px] sm:text-xs font-semibold text-primary dark:text-primary truncate select-all pr-1"
                  title={cat.name}
                >
                  {cat.name}
                </span>
                ...
                {/* Percentage */}
                <span className="text-[11px] sm:text-xs font-bold text-secondary pr-2 text-right shrink-0 select-all">
                  {percentage}%
                </span>

                {/* Amount */}
                <span className="text-[11px] sm:text-xs font-bold font-mono text-primary dark:text-primary/90 text-right shrink-0 select-all">
...
export function RepaymentGauge({ totalDebt, remaining, name, currency = 'Rs.' }: { totalDebt: number; remaining: number; name: string; currency?: string }) {
  ...
  return (
    <div className="bg-card-50 border border-default rounded-[24px] p-4.5 flex gap-4 items-center shadow-xl">
      <div className="relative w-14 h-14 shrink-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[10px] font-mono font-bold text-primary dark:text-primary">
          {percentage}%
        </div>
      </div>
      <div className="min-w-0 flex-1">
        <div className="text-xs font-bold text-primary dark:text-primary truncate">{name} Repaid</div>
