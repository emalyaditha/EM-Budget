...
export function CommandPalette({ isOpen, onClose, onSelectAction }: CommandPaletteProps) {
  ...
  const items: CommandItem[] = [
    ...
    },
    {
      id: 'add-card',
      title: 'Add Bank Card',
      subtitle: 'Register debit or credit card',
      category: 'Actions',
    ...
    },
    {
      ...
      subtitle: 'Net worth & liquidity overview',
      category: 'Navigation',
      icon: <Wallet size={14} />,
    },
    {
      ...
      subtitle: 'Ledger history and filters',
      category: 'Navigation',
    ...
    },
    {
      ...
      subtitle: 'Spending limits and savings',
      category: 'Navigation',
  ...
  const handleKeyDownInMenu = (e: React.KeyboardEvent) => {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setSelectedIndex((p) => (p + 1) % Math.max(1, filteredItems.length));
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setSelectedIndex((p) => (p - 1 + filteredItems.length) % Math.max(1, filteredItems.length));
    } else if (e.key === 'Enter') {
      e.preventDefault();
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-[560px] bg-[var(--surface)] border border-[var(--line)] rounded-2xl shadow-2xl overflow-hidden z-10"
          ...
          >
            <div className="flex items-center gap-3 px-4 h-12 border-b border-[var(--line)] bg-[var(--surface)]">
              ...
              <button
                onClick={onClose}
                aria-label="Close"
                className="w-7 h-7 rounded-full bg-[var(--surface-2)] border border-[var(--line)] text-[var(--ink-2)] hover:text-[var(--ink)] flex items-center justify-center shrink-0"
              >
                <X size={12} />
              </button>
            </div>
                  ...
                  {filteredItems.map((item, index) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between px-3 py-2.5 rounded-xl cursor-pointer border ${isSelected ? 'bg-[var(--surface-2)] border-[var(--line-strong)]' : 'border-transparent hover:bg-[var(--surface-2)] hover:border-[var(--line)]'}`}
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <span
                            className={`w-7 h-7 rounded-full border flex items-center justify-center shrink-0 ${isSelected ? 'bg-[var(--ink)] text-[var(--bg)] border-[var(--ink)]' : 'bg-[var(--surface-2)] text-[var(--ink-2)] border-[var(--line)]'}`}
                          >
                            {item.icon}
                          </span>
